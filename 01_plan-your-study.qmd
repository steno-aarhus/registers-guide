...
10](10_build-study-population.qmd) shows how to build a matched cohort study.
...
**Cohort** follows persons forward in time from the index date and measures how many develop the outcome - which is why you can compute incidence and risk. Well suited when you have multiple outcomes (cf. the `alle_dx` approach in [Extract from LPR](09b_extract-from-lpr.qmd)).
...
With register data you can do both, because the entire population's history is available. [Phase 10](10_build-study-population.qmd) shows a **matched cohort study** step by step.
...
bias](dst-pitfalls.qmd#immortal-time-bias). (The framing fits effect questions,
not purely descriptive or prediction studies.)
:::
...
- **Treatment strategies:** the exposure and its comparator, each defined clearly enough that a person could in principle be assigned either ([Comparison cohort](10a_comparison-cohort.qmd)).
...
all, see [Time-varying variables](12d_time-varying.qmd#feedback).
...
These are the same three conditions restated for weighting on the [IP weighting](13g_weighting-iptw.qmd) page.
</details>
...
- **Who is an appropriate comparator?** Either an **active comparator**
  ...
  the question; expanded in [Comparison cohort](10a_comparison-cohort.qmd).
...
Build your study population](10_build-study-population.qmd).
...
- **Ready for the DST server?** → [Phase 3 - Log in to DST](03_log-in-dst.qmd)
