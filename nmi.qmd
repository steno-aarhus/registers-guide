...
nmi_lmdb <- read_register("lmdb") %>% # without fastreg: open_dataset("path/to/lmdb/")
  rename_with(tolower) %>%
  semi_join(tibble(pnr = pnr_vec), by = "pnr", copy = TRUE) %>% # only the cohort
...
Steps 2.1-2.2 collapse `dx`/`rx` to one row per `pnr`, and 2.3 rejoins onto
`cohort` on `pnr` alone - if your cohort has repeated `pnr` (e.g. a comparison
group matched **with replacement**), that rejoin can attach one person's
flags, and so their whole NMI score, to the wrong `index_date`'s row. See
[comparison cohort's callout on
