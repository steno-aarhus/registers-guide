...
data.frame, which *all* subsequent operations inherit. That means: - `mutate()`
calculates per group, not across the entire dataset - `n()` counts rows per
group, not in total - `summarize()` gives one result per group
