...
1. **fastreg (recommended):** Use `read_register("akm")` - reads by name, fastreg knows the path (see [Phase 4](04_filformater.qmd#convert-sas-to-parquet)).
...
3. **DARTER / project 708421:** Use `load_database("akm")` directly - `dstDataPrep` finds the path automatically.
4. **SAS files (if parquet is not ready):** Use `haven::read_sas("path/akm.sas7bdat")` - but this reads the entire file into RAM. **Recommended:** convert to parquet once and work from that (see [Phase 4 - Convert SAS to parquet](04_filformater.qmd#convert-sas-to-parquet)).
