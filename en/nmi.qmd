...
**two steps - extract, then compute** - kept apart so the slow extraction runs
(and is saved) once, while the compute step, which you re-run each time you
tweak predictors, stays fast.
...
dx <- dx_raw %>%
  ...
  filter(date_contact >= index_date - days(1825),  # 1825 days = 5 years before index
         date_contact <= index_date - days(1)) %>% # up to the day before index (not index day itself)
  # One TRUE/FALSE flag per NMI predictor: str_detect() matches c_diag (with D-prefix) against the predictor's ICD codes
  mutate(
  ...
  #   - one row per pnr, one column per dx_ predictor (~29 of them), each valued 0 or 1
  #   - any() = 1 if the person has at least one matching diagnosis in the 5-year window, else 0
  group_by(pnr) %>%
...
analysis %>%
  ...
  summarise(
    n = n(),
    mean_nmi = mean(nmi, na.rm = TRUE), # na.rm = TRUE: ignore missing scores (NA)
