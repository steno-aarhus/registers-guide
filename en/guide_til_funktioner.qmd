...
You do not need this step with `read_register()` (fastreg) - they hand you a DuckDB connection already, so the `to_duckdb()` conversion is built in. For when to use which, see [Phase 5 - Arrow vs. DuckDB](05_udtraek-trin-for-trin.qmd#arrow-vs-duckdb).
:::
...
Used when you want to save an R dataset as a parquet file, e.g. for use with `open_dataset()` or `read_register()` in another script.
...
**Important:** always call it on the same line as `open_dataset()` (or `read_register()` in DARTER).
