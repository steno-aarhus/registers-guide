...
This page shows the complete process from a raw register to a saved dataset ready for analysis.
You will see the pattern that recurs in almost every register extraction.

::: {.callout-important}
**These examples cannot be run on the DST server.**
The synthetic dataset (`fakeregs`) is not available there.
You need **RStudio installed locally on your computer**:
...
3. Open RStudio, create a new script (**File → New File → R Script**), and copy the code there.
...
::: {.callout-note}
**The examples use synthetic data from the package [`fakeregs`](https://github.com/steno-aarhus/fakeregs)** (MIT licence, Anders Aasted Isaksen, Steno Diabetes Center Aarhus) - fictitious persons with the structure and column names of DST registers.
The package uses `generate_*()` functions to create synthetic data, which you save as parquet and practise `open_dataset()` on. (These local examples use `open_dataset()`; fastreg's `read_register()` is for a configured DST project, not ad-hoc local folders.)
:::


::: {.callout-note}
**Next step: hospital diagnoses (LPR)**
This example uses `lpr_adm` (hospital contacts - contact dates only, no diagnosis join). When working with **diagnosis data from LPR** (ICD codes), additional rules apply: the register is split into two periods (LPR2 and LPR3), codes have a D-prefix that must be removed, and you must choose diagnosis types. All of this is covered in [Understand LPR](09a_forstaa_lpr.qmd).
:::
...
`"synth_data/bef/"` is created in the folder R is set to work in - check which one with `getwd()`.
To save elsewhere, use a full path: `"C:/Users/yourname/projects/synth_data/bef/"`.
:::
...
Every extraction starts with a list of pnr's - the people you want data for.
In practice this comes from your cohort script. Here we build a small practice cohort
directly from the BEF register.
...
::: {.callout-tip}
In a real project, `cohort_pnrs` is a vector you built in a previous script
and reload with `readRDS("sti/til/full_cohort.rds") %>% pull(pnr)`.
:::

::: {.callout-note}
**What do the signs in `filter()` mean?** `,`/`&` = AND, `\|` = OR, `!` = NOT, plus `==`, `!=`, `>`, `>=`, `%in%`. Full overview with examples and the parentheses pitfall: [Combining conditions in `filter()`](guide_til_funktioner.qmd#filtrer-betingelser).
:::
...
This is only an excerpt. Isaksen's full vignette is more thorough and covers additional variables and patterns -
see it directly here: [steno-aarhus.github.io/fakeregs/articles/common_tasks_dplyr.html](https://steno-aarhus.github.io/fakeregs/articles/common_tasks_dplyr.html)
...
Now we extract hospital contacts from `lpr_adm` for our cohort.
The pattern is always the same: **open → filter → select columns → collect**.
...
See [Extracting data step by step](05_udtraek-trin-for-trin.qmd) for a detailed explanation of lazy evaluation.
...
Intermediate results are repatriated via output control - see [14 - Export and repatriation](14_eksport-hjemsendelse.qmd).
:::
...
- [Phase 7 - Inspect your data](07_inspicer-data.qmd): the full toolkit for understanding what you have
- [Extract from LPR](09b_udtraek_fra_lpr.qmd): the complete pattern for diagnosis extractions
- [Phase 12 - Assemble and prepare the dataset](12_data-manipulation.qmd): combine two extracts

---

## Source and adaptation

Step 1 (cohort construction from BEF) is adapted from Anders Aasted Isaksen's vignette
[`common_tasks_dplyr.qmd`](https://github.com/steno-aarhus/fakeregs/blob/main/vignettes/common_tasks_dplyr.qmd)
in the `fakeregs` package (MIT licence, Steno Diabetes Center Aarhus).
Steps 2–4 and the checklist are written for this guide.
