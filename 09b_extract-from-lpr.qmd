...
structure from [Understand LPR](09a_understand-lpr.qmd): the periods
...
5](05_extraction-step-by-step.qmd) and [Phase 6](06_first-extraction.qmd) - just
...
10](10_build-study-population.qmd) using exactly the extraction pattern you
learn here. Read the phases in order, and come back to the code when your cohort
is ready. The code uses `inner_join()` and `bind_rows()`; if these are new, they
...
- **Use the LPR_A files consistently.** DST has switched LPR3 from the old
  **LPR_F** format (`kontakter`, `diagnoser`, `forloeb`) to the new **LPR_A**
  ...
  5](darter/02_darter-pitfalls.qmd#lpr3-lprindberetningssystem).
...
- **Mind the 2019-2020 diagnosis spike.** The LPR3 contact model registers far
  ...
  diagnosis spike](09a_understand-lpr.qmd#lpr3-spike).
:::
...
10](10_build-study-population.qmd).
...
# LPR2 psychiatric (up to March 2019) - OPTIONAL -------------------------------
# Only if your outcome is an F-code (dementia, depression, ...)
...
psych registers `t_psyk_adm`/`t_psyk_diag`. The `rename()` calls above assume the
DARTER column names `v_cpr`, `k_recnum` and `v_recnum`; DST's own lists say `pnr`
and `recnum`, so check `colnames()` and drop the renames if you already have
DST's names. They are included in the flow above. Omit them
...
::: {.callout-note}
Exclusion of prevalent cases - persons who already had the diagnosis before index date - happens in [Phase 10, Step 3](10_build-study-population.qmd). Use `cohort_clean` instead of `cohort` in the code above after completing that step.
:::
...
The synthetic LPR data is generated with the `fakeregs` package, which you already know from [Phase 6 - First extraction](06_first-extraction.qmd#preparation-install-fakeregs-and-generate-synthetic-data). If you have already generated and saved data there, `synth_data/lpr_adm/` is ready and you can skip the preparation block.
...
If you have already run the preparation block in [Phase 6](06_first-extraction.qmd), `synth_data/lpr_adm/` is already saved.
...
analysis (see [Diagnosis types](09a_understand-lpr.qmd#diagtype)) without
re-querying LPR.

**Advantages:**

- One place to fix if something changes (e.g. a new register or a new column)
- The code block for each outcome is reduced from \~40 lines to one function
  call
...
memory with `rm()` + `gc()` ([Phase 5](05_extraction-step-by-step.qmd)), then filter
...
Run `colnames()` on your own `lpr_adm` before writing this code. If the columns
are missing, no filtering trick recovers them: you have to ask DST to add them to
the delivery.
:::
...
- [Understand LPR](09a_understand-lpr.qmd): structure, periods, D-prefix and
  diagnosis types
- [Phase 6 - First extraction](06_first-extraction.qmd): step-by-step
