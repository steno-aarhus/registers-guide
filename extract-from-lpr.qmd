...
10](build-study-population.qmd) using exactly the extraction pattern you
...
are explained in detail in [Phase 12](data-manipulation.qmd).
:::
...
- **Mind the 2019-2020 diagnosis spike.** The LPR3 contact model registers far
  ...
  diagnosis spike](understand-lpr.qmd#lpr3-spike).
:::
...
::: {.callout-note}
Exclusion of prevalent cases - persons who already had the diagnosis before index date - happens in [Phase 10, Step 3](build-study-population.qmd). Use `cohort_clean` instead of `cohort` in the code above after completing that step.
:::
...
analysis (see [Diagnosis types](understand-lpr.qmd#diagtype)) without
re-querying LPR.
