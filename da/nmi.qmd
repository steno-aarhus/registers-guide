...
- Flere almindelige kroniske sygdomme er ikke inkluderet i modellen (fx
  hypertension og type 1-diabetes).
...
kører i **to trin - udtræk, dernæst beregn.** Kodeeksemplerne bygger på
**Pernille Errebo Tonnesens** oversættelse af SDU's Stata-kode til R.
...
*Diagnoser* - Du kan bruge de to tilgange fra [Udtræk fra
LPR](09b_udtraek_fra_lpr.qmd): enten udtrække **alle** diagnoser og ATC-koder
for hele kohorten (anbefales ikke - tids-, RAM- og pladskrævende, især ved en
stor kohorte), eller **filtrere til kun de koder NMI skal bruge før
`collect()`**, så de gemte `.rds`-filer holdes små.

Du kan også benytte `get_lpr_diagnoses()`-mønstret fra [Udtræk fra
LPR](09b_udtraek_fra_lpr.qmd) som i eksemplet herunder:
...
<details>
<summary>Se koden: beskriv NMI-scoren</summary>
