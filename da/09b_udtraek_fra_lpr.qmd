...
- **Brug LPR_A-filerne konsekvent.** DST har skiftet LPR3 fra det gamle
  ...
  **LPR_A**-format (`lpr_a_kontakt`, `lpr_a_diagnose`). Begge versioner kan
  ...
  eksemplerne her, og lad LPR_F-filerne ligge. Nogle projekter har desuden ældre
  data *inde i* `lpr_a_kontakt`, som allerede findes i LPR2; filtrér dem fra med
  `lprindberetningssystem == "LPR3"`. Det er projekt-specifikt - på **DARTER**
  er det håndteret i [faldgrube
  5](darter/02_darter-faldgruber.qmd#lpr3-lprindberetningssystem); ellers afklar
  med din datamanager.
- **Verificér kolonnenavne.** Skiftet til LPR_A har ændret en række
  ...
  til de DuckDB-baserede objekter fra `read_register()`/`read_register()`). Slå
  navnene op i [Register-overblik](register_reference.qmd).
