...
subtitle: "Incidensrate-ratio (IRR) med Poisson-regression og person-tid - og hvornår du vælger den frem for Cox"
date: last-modified
---
...
I [Deskriptive tabeller](13b_deskriptive-tabeller.qmd#crude-rate) beregnede du en **rå (crude)** rate per gruppe. Denne side går skridtet videre: en **Poisson-regression** giver en IRR, der er **justeret for confounding** (alder, køn, andre covariater), på samme måde som logistisk regression giver en justeret odds ratio.
...
- **Logistisk regression** ([Regression](13d_regression.qmd)) ser kun på **ja/nej** og ignorerer tid. Giver en **odds ratio**. Passer, når alle er fulgt lige længe, eller når tid er ligegyldig.
- **Cox-regression** ([Time-to-event](13e_time-to-event.qmd)) ser på **tiden indtil den første hændelse** per person og giver en **hazard ratio**. Den modellerer raten uden at antage en bestemt form på den over tid.
...
Du skal bruge to ting per person, præcis som ved [den crude rate](13b_deskriptive-tabeller.qmd#crude-rate): **antal hændelser** (`event`, her 0 eller 1) og **person-tid** (`followup_years`, opfølgningstid i år). Begge bygges i [Fase 12](12b_joins.qmd#opfoelgningstid).
...
- `offset(log(followup_years))` er person-tiden. Den skal **logaritmeres** (`log()`), fordi Poisson-modellen regner på log-skala. Uden offset modellerer du antallet af hændelser, ikke raten - og det er næsten aldrig det, du vil.
- `exponentiate = TRUE` viser **rate-ratios (IRR)** i stedet for log-rate-koefficienter.

En IRR læses præcis som en OR eller HR: **1 = ingen forskel**, over 1 = højere rate, under 1 = lavere. Se [Sådan læser du dit resultat](13d_regression.qmd#laes-resultat).

::: {.callout-note}
Her er `event` 0/1 (én række per person), og offset er personens samlede opfølgningstid. Det giver en gyldig IRR justeret for de covariater, du har på **baseline** (ved studiestart/index). Vil du lade raten **ændre sig med alderen eller kalenderåret** undervejs i opfølgningen, skal person-tiden splittes - se [Rater der varierer over tid](#rater-over-tid) nedenfor.
:::
...
- **Uafhængige observationer.** Én række (eller én klynge person-tid) per person. Optræder samme person flere gange (fx matchet *med replacement* eller gentagne hændelser), så brug **klyngebaserede standardfejl** ligesom i [Regression](13d_regression.qmd#klyngebaserede-standardfejl).
- **Log-linearitet.** Kontinuerte variable hænger lineært (i en ret linje) sammen med log-raten. Tjek som ved logistisk regression, fx med [splines](13d_regression.qmd#splines).
...
Det er et standardgreb i registerepidemiologi, men kræver lidt mere maskineri, end denne side dækker. Brug `survival::survSplit()` eller `Epi::splitLexis()` (Lexis-objekter), eller pakken `popEpi`, der er bygget til netop rater, person-tid og standardisering på registerdata. Se [Læs mere](#laes-mere).
...
- *The Epidemiologist R Handbook*, generel uddybning (på engelsk): [Univariate and multivariable regression](https://www.epirhandbook.com/en/new_pages/regression.html)
- [`Epi`-pakken](https://CRAN.R-project.org/package=Epi) - person-tid, Lexis-objekter og rater på registerdata.
- [`popEpi`-pakken](https://CRAN.R-project.org/package=popEpi) - rater, SIR/SMR og standardisering, bygget til registerdata.
- [Bendix Carstensens kursusmateriale](http://bendixcarstensen.com/) (Statistical Practice in Epidemiology) - grundig gennemgang af person-tid, alders/periode-split og Poisson-rater.
:::
