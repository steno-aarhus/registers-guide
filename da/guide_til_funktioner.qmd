...
`read_register()` finder stien fra din projekt-config (sat én gang) - se [Fase 4](04_filformater.qmd#konvertér-sas-til-parquet). Den returnerer en DuckDB-forbindelse.
...
De bekræftede stier til dit projekt finder du i [Register-overblik](register_reference.qmd).
...
Det skal du ikke med `load_database()` (dstDataPrep) eller `read_register()` (fastreg) - de giver dig allerede en DuckDB-forbindelse, så `to_duckdb()`-konverteringen er indbygget. Hvornår du bruger hvad, kan du læse i [Fase 5 - Arrow vs. DuckDB](05_udtraek-trin-for-trin.qmd#arrow-vs-duckdb).
:::
...
::: {.callout-note}
Konceptet med doven evaluering - hvorfor data ikke er i hukommelsen før `collect()` - er forklaret grundigt på siden [Udtræk trin for trin](05_udtraek-trin-for-trin.qmd).
:::
...
Se [Filformater](04_filformater.qmd) og [Formateringstabeller](formateringstabeller.qmd) for praktiske eksempler.
...
Glemmer du det, vil `semi_join(..., by = "pnr")` fejle, fordi kolonnen måske hedder `PNR`.
...
`icd3 %in% c("G30", "F00", "F03")` er som at stille sig ved indgangen og tjekke: "er denne diagnosekode på listen?"
...
Se [SEPLINE](11b_socioekonomiske-variable.qmd).
:::
...
**Anekdote:** Forestil dig at du beder en assistent om at finde alle rækker med en kode fra en liste.
Hvis du siger "find alle med en kode fra listen `mine_atc`", vil assistenten kigge efter en kolonne i databasen med det navn - og den eksisterer ikke.
Du skal sige: "find alle med en kode fra *denne* liste" og holde listen frem.
...
- **Alder ved index** (`/365.25`, skudår): [Sammenligningskohorte](10a_sammenligningskohorte.qmd)
- **Baseline-år for årsregistre** (`year(index_date) - 1`): [Socioøkonomiske variable](11b_socioekonomiske-variable.qmd)
- **SAS-heltalsdatoer** (`origin = "1960-01-01"`): [Faldgruber på DST](dst_faldgruber.qmd)
- **Opfølgningstid og hændelsesvariabel** (`pmin()`): [Joins](12b_joins.qmd#opfoelgningstid)
- **Start/stop-format** (tidsvarierende kovariat): [Tidsvarierende variable](12d_tidsvarierende.qmd)
:::
...
Se [Register-overblik](register_reference.qmd) for bekræftede kolonnenavne på alle registre disse pakker arbejder mod.

| Pakke | Hvad den leverer |
|---|---|
| `dstDataPrep` | `load_database()` - DST-specifik parquet-forbinder *(kun DARTER-projektet; den generelle CRAN-ækvivalent er `fastreg::read_register()`)* |
