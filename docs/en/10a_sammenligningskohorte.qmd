...
::: {.callout-note}
**Functions used here.**
`anti_join()` (keep rows *without* a match) is new here — see the explanation in the code block and in [Phase 12](11_data-manipulation.qmd#joins-link-two-tables). `inner_join()`, `bind_rows()` and `group_by() + slice()` are shown in [Phase 9](09b_udtraek_fra_lpr.qmd) and [Phase 12](11_data-manipulation.qmd).
:::

::: {.callout-tip}
**A little theory before you match.** Take an explicit position on:

- **Matching variables:** typically sex, birth year and calendar year; often also SES and municipality. Match only on what you want to adjust for by design.
- **Risk-set / incidence-density matching:** a comparator must be at risk **on the assigned index date** — not only at the start of the study. A person can be a comparator now and become exposed (a case) later.
- **Varying vs. fixed index date:** when the index date varies per exposed person (e.g. each new T2D patient gets their own date), the comparator must be matched on *exactly that date* — which is precisely what risk-set sampling does.
- **With or without replacement:** can a person be a comparator for several exposed individuals? Replacement increases the effective sample size but gives correlated observations that must be handled in the analysis.
:::
...
| Comorbidity (NMI, Charlson) | [NMI](14c_nmi.qmd) |
