...
::: {.callout-note}
**Functions used here.**
`anti_join()` (keep rows *without* a match) is new here — see the explanation in the code block and in [Phase 11](11_data-manipulation.qmd#joins-link-two-tables). `inner_join()`, `bind_rows()` and `group_by() + slice()` are shown in [Phase 9](09b_udtraek_fra_lpr.qmd) and [Phase 11](11_data-manipulation.qmd).
:::
