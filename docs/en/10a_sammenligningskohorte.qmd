...
**You must first build `diagnoses` (and the exposure source) as shown in [Extract from LPR (9b)](09b_udtraek_fra_lpr.qmd) - it is not a raw table.** The diagnosis registers (`lpr_diag`, `lpr_a_diagnose`) have neither `pnr` nor a date, so you cannot just open them and filter. You extract **one register at a time** and **combine** them at the end, exactly as in 9b:
