...
::: {.callout-tip}
**Not matching the group?** You can also use an **unmatched** comparison group and instead adjust for the confounders in the analysis. In that case, skip to the section [Without matching](#without-matching-an-unmatched-comparison-group) at the bottom of the page.
:::
...
::: {.callout-note}
**Create a derived variable.** A new variable is computed from existing columns with `mutate()` (expanded in the [Function guide](15a_guide_til_funktioner.qmd)). Age at index = number of days between index and birth, divided by 365.25 (a year is 365.25 days because of leap years):

```r
mutate(age_at_index = as.numeric(index_date - foed_dag) / 365.25)
```

The same principle applies to other derived measures, e.g. BMI from height and weight: `mutate(bmi = weight_kg / (height_m^2))`. Compute the variable ONCE and reuse it, so the matching age and Table 1 always agree. See [Phase 12](11_data-manipulation.qmd#derived-variables-create-new-columns) for more derived variables (categorizing, event indicator).
:::
...
::: {.callout-important}
**With replacement, be careful in the analysis.** Standard models assume each row is an **independent** observation. When the same person appears several times (reuse or crossover), that does not hold - the rows are **correlated**, and the model "thinks" it has more independent information than it does. The result is **confidence intervals that are too narrow** (and p-values too small). The fix is **clustered (robust) standard errors**: you tell the model which rows belong to the same person (clustering on person id), so the uncertainty is computed correctly - or you use a model that handles it. This is an **analysis step** to remember; it is expanded in [Phase 13 - Analysis](13_analyse.qmd) (the section is not yet written).
:::
