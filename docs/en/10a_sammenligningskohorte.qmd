...
# --- Pull BEF person-years ONCE and reuse it (for both the exposed AND the pool in Step 4) ---
bef_py <- bef %>%
...
# --- Attach to the exposed (comes from Phase 10: identified + prevalent cases excluded) ---
exposed <- exposed %>%
...
stopifnot(n_distinct(exposed$pnr) == nrow(exposed))  # check: still one row per person
```
...
```r
# Dates extracted directly for the population (same patterns as Phase 9; here as finished tables)
death   <- readRDS("path/to/death_dates.rds")     # pnr, death_date         (DODSAARS, exact date of death)
emig    <- readRDS("path/to/emigration.rds")      # pnr, emigration_date    (VNDS, emigration date)
outcome <- readRDS("path/to/outcome_dates.rds")   # pnr, first_outcome_date (FIRST outcome ever)
