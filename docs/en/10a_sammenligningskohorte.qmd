...
::: {.callout-note}
**Functions used here.**
`left_join()` (attach a variable) and `semi_join()` (keep only rows *with* a match) are explained in [Link your extracts](12b_joins.qmd#joins-link-two-tables). `mutate()` (create a new variable) is shown in [Phase 2](02_r-intro.qmd). New here: a `for` loop + `set.seed()` for the matching itself (explained in Step 5). Functions like `slice_sample()`, `count()` and `nrow()` are in the [Function guide](guide_til_funktioner.qmd).
:::
...
*(Steps 1 and 2 - identify the exposed and exclude prevalent cases - are on the [landing page](10_byg-studiepopulation.qmd).)*
...
::: {.callout-note}
**Create a derived variable.** A new variable is computed from existing columns with `mutate()` (expanded in the [Function guide](guide_til_funktioner.qmd)). Age at index = number of days between index and birth, divided by 365.25 (a year is 365.25 days because of leap years):

```r
mutate(age_at_index = as.numeric(index_date - foed_dag) / 365.25)
```

The same principle applies to other derived measures, e.g. BMI from height and weight: `mutate(bmi = weight_kg / (height_m^2))`. Compute the variable ONCE and reuse it, so the matching age and Table 1 always agree. See [Phase 12](12_data-manipulation.qmd#derived-variables-create-new-columns) for more derived variables (categorizing, event indicator).
:::
...
Each date table ends up as **one row per person** (just the date eligibility depends on) - which is why the pool join below keys on `pnr`. The extract is the **same kind as in [Phase 9](09b_udtraek_fra_lpr.qmd)**; the only difference is that you do **not** restrict to the exposed persons' `pnr`: you keep everyone, because a comparison person can be anyone.

**Important about `diagnoses`:** it is the **combined, harmonized** LPR diagnosis table from Phase 9 - not a raw table. It is built there by joining `lpr_adm`+`lpr_diag` on `recnum` (LPR2) and `lpr_a_kontakt`+`lpr_a_diagnose` on `dw_ek_kontakt` (LPR3), combining psychiatry and somatics, and harmonizing the date column to `date_contact` (renamed from `d_inddto` / `kont_starttidspunkt`). For the whole population you run exactly that [9b extract](09b_udtraek_fra_lpr.qmd), just **without** `filter(pnr %in% kohort_pnrs)`. So the real join keys live in 9b (`recnum`/`dw_ek_kontakt`); here in the pool join the key is `pnr`, because the tables are now one row per person.
...
# Same patterns as in Phase 12 - note: NO filter(pnr %in% exposed$pnr).
...
# Save them as .rds so they can be reused later - exactly the files the code block below can load again:
...
| Extraction | Phase |
|---|---|
| Outcomes (diagnoses, date of death, emigration) | [Extract from LPR](09b_udtraek_fra_lpr.qmd), [Outcomes](11a_udfald.qmd) |
...
- [Build your study population](10_byg-studiepopulation.qmd) - identify the exposed (Step 1) and exclude prevalent cases (Step 2)
...
- [Phase 12 - Assemble and prepare the dataset](12_data-manipulation.qmd) - assemble all extracts into one dataset
