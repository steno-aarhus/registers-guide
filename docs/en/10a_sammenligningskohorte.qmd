...
This is the **same kind of extract as in Step 2 / [Phase 9](09b_udtraek_fra_lpr.qmd)** - the only difference is that you do **not** restrict to the exposed persons' `pnr`: you keep everyone, because a comparison person can be anyone. Each table ends up as one row per person (just the date eligibility depends on).

```r
library(arrow); library(dplyr)

# --- First outcome date for EVERYONE ---
# 'diagnoses' is your LPR diagnosis extract (pnr, date_contact, icd3) from Phase 9.
# Querying on the outcome codes returns ALL persons with the diagnosis - regardless of exposure.
# (In Step 2 we joined to the exposed; here we simply aggregate for everyone.)
outcome <- diagnoses %>%
  filter(icd3 %in% OUTCOME_CODES) %>%                # same outcome codes as in Step 2
  group_by(pnr) %>%
  summarise(first_outcome_date = min(date_contact), .groups = "drop")  # earliest outcome per person
...
exp_dt <- exposure_raw %>%                            # your exposure extract, one row per exposure event
  group_by(pnr) %>%
  summarise(exposure_date = min(exposure_date), .groups = "drop")      # first exposure per person
