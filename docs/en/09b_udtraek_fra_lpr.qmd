...
::: {.callout-tip}
**Using fastreg, or working on DARTER?** With **fastreg**, replace `open_dataset("E:/workdata/.../<register>/")` with `read_register("<register>")` (by name). On **DARTER**, use `load_database("<register>")` from `dstDataPrep` instead - it resolves the path automatically. See [DARTER - overview and pipeline](darter/00_index.qmd) for the fully adapted variant - it is kept up to date with the current, confirmed register names (as of June 2026).
:::
