...
```{mermaid}
flowchart TD
    A{"How many outcomes<br>from LPR?"}
    A -->|"One / few"| B["Approach 1<br>direct extraction"]:::a1
    A -->|"Several"| C["Approach 2<br>build alle_dx once"]:::a2
    B --> D{"F-codes?<br>(psychiatry, e.g. dementia)"}
    C --> D
    D -->|"No"| E["lpr_adm/lpr_diag +<br>lpr_a_kontakt/lpr_a_diagnose"]:::plain
    D -->|"Yes"| F["Also add<br>t_psyk_adm / t_psyk_diag"]:::plain

    classDef a1 fill:#eaf2fb,stroke:#4a78b5,color:#173a5e;
    classDef a2 fill:#e9f7ef,stroke:#3fae6b,color:#14532d;
    classDef plain fill:#f6f6f6,stroke:#aaaaaa,color:#444444;
```
