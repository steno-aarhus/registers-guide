...
You arrive at this phase with separate RDS files: your cohort from [Phase 10](10_byg-din-kohorte.qmd), event dates from [Phase 9](09_hospitalskontakter-lpr.qmd), covariates from [Phase 6](06_foerste-udtraek.qmd) and socioeconomic variables from [Phase 12](12_socioekonomiske-variable.qmd). You leave with one analysis-ready dataset ready for [Phase 13](13a_pakker-oversigt.qmd).
...
Instead use the pattern from [Phase 9](09_hospitalskontakter-lpr.qmd): extract the first date for one specific diagnosis group, save as `extract_dementia.rds`, and join it as one column with `left_join()`. One outcome variable = one column = one RDS file.
:::
...
1. **Define the exposed cohort.** Filter the register that defines the exposure — e.g. bariatric surgery via SKS codes in `lpr_sksopr` / `procedurer_kirurgi` ([Phase 9](09_hospitalskontakter-lpr.qmd)). Keep `pnr` and **index date** (the surgery date).
...
Functions used here are explained in [Phase 14 — Functions: overview](14a_guide_til_funktioner.qmd) (`group_by`, `slice`, `transmute`, `left_join`) and [Phase 13](13a_pakker-oversigt.qmd) (`riskSetMatch`).
...
→ [Phase 12 — Socioeconomic variables](12_socioekonomiske-variable.qmd)

## See also

- [Phase 14 — Functions: overview](14a_guide_til_funktioner.qmd) — all functions for transforming data
- [Phase 9 — Hospital contacts (LPR)](09_hospitalskontakter-lpr.qmd) — practical example of long register data and inner_join
