...
You arrive at this phase with separate RDS files: your cohort from [Phase 10](10_byg-din-kohorte.qmd) and the variables you extracted in [Phase 11](11_foej-variabler.qmd) - outcomes, socioeconomics and comorbidity (plus demographics from [Phase 6](06_foerste-udtraek.qmd) if relevant). You leave with one analysis-ready dataset ready for [Phase 13 - Analysis](13_analyse.qmd).
...
- **Follow-up time** (`censor_date - index_date`) is usually computed *after* the join, because it uses dates from several extracts - see [12b](12b_joins.qmd).

::: {.callout-tip}
`mutate()`, `if_else()` and `case_when()` are explained in the [Function guide](15a_guide_til_funktioner.qmd).
:::
...
1. **[12a - Long ↔ wide format](12a_langt-bredt.qmd)** - get each extract down to **one row per person**. Understand the difference between long and wide format, and use either `group_by() + slice()` (one representative value per person) or `pivot_wider()` (several time points side by side).
2. **[12b - Link your extracts](12b_joins.qmd)** - join the extracts on `pnr` (`left_join()`, `inner_join()`, `semi_join()`, `anti_join()`), stack two versions of the same register vertically (`bind_rows()`), validate the join, and calculate follow-up time and event variable. **This is where the final dataset is assembled.**
...
→ [12a - Long ↔ wide format](12a_langt-bredt.qmd)
