...
The goal of this phase is to assemble them into one dataset with `left_join()` — one new column per file.

```{mermaid}
flowchart LR
    E1["extract_dementia.rds"]:::ext
    ...
    A[("analysis_data<br>one row per person")]:::out
...
The cohort is the backbone. Each extract is joined on with `left_join()` via `pnr` and adds its columns — all cohort members are kept, and people with no match get `NA`.
...
## Practical example: cohort + date of death
...
## Practical example: cohort + emigration date
...
The actual **matching** with `heaven::riskSetMatch()` is shown in the code skeleton below.
:::
...
<details>
<summary>More in depth — matching, immortal time and a code skeleton</summary>

**Important pitfalls when building the comparator cohort:**

- **Immortal time bias:** a person in the comparator cohort must be alive, residing in Denmark and meeting the inclusion criteria **on their assigned index date** — not only at study start.
- **Risk-set / incidence-density sampling:** a person can appear in the comparator cohort and *later* themselves become operated and become a case. Decide how you handle that (heaven supports risk-set matching).
- **Matching ratio:** e.g. 1:5 exposed:comparator cohort — more controls gives more precision, but with diminishing returns.
- **The same exclusions** are applied to both groups, otherwise you introduce selection bias.

**Code skeleton** (pattern — not a finished pipeline):

```r
library(dstDataPrep); library(dplyr); library(heaven)

# 1. Exposed cohort: surgery date as index (SKS codes — see Phase 9)
exposed <- opr_all %>%                       # from the SKS extraction (Phase 9 / 14d)
  group_by(pnr) %>% arrange(surgery_date) %>% slice(1) %>% ungroup() %>%
  transmute(pnr, index_date = surgery_date, exposed = 1L)

# 2. Matchpool from BEF (all who were NOT operated on) — add matching variables
#    (age at index, sex, calendar year) ...

# 3. Risk-set matching, e.g. 1:5 on sex + birth year
...
::: {.callout-warning}
**The skeleton is a starting point — use it as inspiration.** Steps 1–3 above can be used now: replace variable names and codes with your own. The full, project-specific and tested pipeline for this study is not yet finished.
:::

Functions used here are explained in [Phase 15 — Functions: overview](15a_guide_til_funktioner.qmd) (`group_by`, `slice`, `transmute`, `left_join`) and [Phase 14](14a_pakker-oversigt.qmd) (`riskSetMatch`).
