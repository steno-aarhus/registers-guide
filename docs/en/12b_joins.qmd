...
1. **Define the exposed cohort.** Filter the register that defines the exposure - e.g. bariatric surgery via SKS codes in `lpr_sksopr` / `procedurer_kirurgi` ([Extract from LPR](09b_udtraek_fra_lpr.qmd)). Keep `pnr` and **index date** (the surgery date).
...
The actual **cohort building and matching** with `heaven::riskSetMatch()` - including pitfalls such as immortal time - are in [Phase 10 - Build your study population](10_byg-studiepopulation.qmd).
:::
