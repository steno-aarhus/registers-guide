...
::: {.callout-note}
**What do the signs in `filter()` mean?** `,`/`&` = AND, `\|` = OR, `!` = NOT, plus `==`, `!=`, `>`, `>=`, `%in%`. Full overview with examples and the parentheses pitfall: [Phase 15a — Combining conditions in `filter()`](15a_guide_til_funktioner.qmd#filtrer-betingelser).
:::
...
Intermediate results are repatriated via output control — see [16 — Export and repatriation](16_eksport-hjemsendelse.qmd).
:::
...
- [Phase 11 — Joins and pivots](11_data-manipulation.qmd) — combine two extracts
