...
This example uses `lpr_adm` (hospital contacts - contact dates only, no diagnosis join). When working with **diagnosis data from LPR** (ICD codes), additional rules apply: the register is split into two periods (LPR2 and LPR3), codes have a D-prefix that must be removed, and you must choose diagnosis types. All of this is covered in [Phase 9 - Hospital contacts (LPR)](09a_forstaa_lpr.qmd).
:::
...
::: {.callout-note}
**What do the signs in `filter()` mean?** `,`/`&` = AND, `\|` = OR, `!` = NOT, plus `==`, `!=`, `>`, `>=`, `%in%`. Full overview with examples and the parentheses pitfall: [Combining conditions in `filter()`](15a_guide_til_funktioner.qmd#filtrer-betingelser).
:::
...
See [Extracting data step by step](05_universelle-moenster.qmd) for a detailed explanation of lazy evaluation.
...
Intermediate results are repatriated via output control - see [14 - Export and repatriation](16_eksport-hjemsendelse.qmd).
:::
...
- [Phase 9 - Hospital contacts (LPR)](09b_udtraek_fra_lpr.qmd) - the complete pattern for diagnosis extractions
- [Phase 12 - Assemble and prepare the dataset](11_data-manipulation.qmd) - combine two extracts
