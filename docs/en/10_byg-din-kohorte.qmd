...
![Timeline: the exposed person's index date (exposure date) is assigned to the matched comparison person. Follow-up runs from index to event or censoring; covariates are measured in the baseline window before index.](images/index_matching.svg){fig-alt="Timeline with an exposed and a comparison person sharing an index date; follow-up up to event or censoring." width="100%"}
