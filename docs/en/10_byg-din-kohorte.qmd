...
- **Prevalence study / cohort without a comparison group** - you are done here; continue to [Phase 11 - Extract variables](11_foej-variabler.qmd) and [Phase 12](11_data-manipulation.qmd).
...
::: {.callout-note}
**Functions used here.**
`inner_join()` and `bind_rows()` are shown in [Phase 9](09b_udtraek_fra_lpr.qmd). `group_by() + slice()` is shown briefly there and explained in detail in [Phase 12](11_data-manipulation.qmd). New in Step 2: `anti_join()`, `distinct()`, `pull()` plus `nrow()`, `cat()` and `stopifnot()` - see [Function guide](15a_guide_til_funktioner.qmd).
:::
...
::: {.callout-note}
**A small helper function.** If the same check is needed more than once (e.g. also on a comparison group later, see [Phase 10a](10a_sammenligningskohorte.qmd)), wrap the logic in a function so the two checks do not drift apart. See [Good coding practice](12_god-kode-praksis.qmd).
:::
...
- **Prevalence study / cohort without a comparison group** - you now have your study population with an index date. Continue to [Phase 11 - Extract variables](11_foej-variabler.qmd) and [Phase 12 - Assemble and prepare the dataset](11_data-manipulation.qmd).
- **Cohort study with a comparison cohort** → [Comparison cohort](10a_sammenligningskohorte.qmd) - build a match pool and risk-set match a comparison group.
