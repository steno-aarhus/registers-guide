...
All extractions in the other phases — outcomes ([Phase 9](09_hospitalskontakter-lpr.qmd)), covariates ([Phase 6](06_foerste-udtraek.qmd)) and socioeconomics ([Phase 12](12_socioekonomiske-variable.qmd)) — assume you already have a cohort: a table with `pnr` and `index_date` per person. This page shows how to build it from scratch.
...
::: {.callout-tip}
Design choices behind index date, immortal time and the relationship between eligibility criteria and the exposure are covered in [Phase 1 — Study preparation](01_studieforberedelse.qmd).
:::
...
^a^ `lpr_sksopr` and `procedurer_kirurgi` are the names on the DARTER project (708421) — see [Register reference](14d_register_reference.qmd). Names may vary on other projects.
...
::: {.callout-tip}
`exposed` contains only the operated individuals. The full cohort (exposed + comparator cohort) is built in step 4 and saved as `cohort`. It is `cohort` — not `exposed` — that you use as `cohort_pnrs` in the other phases.
:::

### Example B: ICD diagnosis as exposure criterion

```r
# Same pattern as Phase 9 — here without prior cohort_pnrs filter
...
- **Prevalence study** — you now have your study population with an index date. Skip steps 2–4 and go directly to [Phase 6 — Extract covariates](06_foerste-udtraek.qmd) and [Phase 11 — Assemble the dataset](11_data-manipulation.qmd).
- **Cohort study or nested case-control** — continue with steps 2–4 below.

*Note for case-control:* your "exposed" group in step 1 is your **cases** (those who experienced the outcome, identified from LPR), not an exposure. Steps 2–4 then sample **controls** — people without the outcome who were at risk at the time each case received their diagnosis.
:::

## Step 2 — Build matchpool from BEF
...
## Step 3 — Apply inclusion criteria BEFORE sampling

::: {.callout-important}
**Inclusion criteria must be applied BEFORE matching — not after.**
Exclusion of e.g. persons under 18, not residing in Denmark, or with prevalent outcome must happen on the **matchpool** before comparators are sampled. Excluding after matching introduces systematic bias, because you change the population that the exposed are matched to.
...
# Apply the same criteria to the exposed:
exposed_clean <- exposed %>%
  inner_join(bef_baseline, by = "pnr") %>%
  filter(alder >= 18, opr_land == 5100)
```

For exclusion of prevalent diagnoses (e.g. existing dementia before index date) — see [Phase 9 — Exclude prevalent cases](09_hospitalskontakter-lpr.qmd#exclude-prevalent-cases). Prevalence exclusion must likewise happen **before** sampling.

---

## Step 4 — Match comparators
...
`riskSetMatch()` is explained in [Phase 13a — Packages](13a_pakker-oversigt.qmd) and the design choice behind matching in [Phase 1](01_studieforberedelse.qmd#comparator-cohort).
:::
...
| Extraction | Phase |
|---|---|
| Outcomes (diagnoses, date of death, emigration) | [Phase 9](09_hospitalskontakter-lpr.qmd), [Phase 11](11_data-manipulation.qmd) |
| Covariates from BEF (age, sex) | [Phase 6](06_foerste-udtraek.qmd) |
| Socioeconomic variables | [Phase 12](12_socioekonomiske-variable.qmd) |
| Comorbidity (NMI, Charlson) | [Phase 13c](13c_nmi.qmd) |
...
- [Phase 9 — Hospital contacts (LPR)](09_hospitalskontakter-lpr.qmd) — diagnosis pattern for exposure identification and prevalence exclusion
- [Phase 11 — Joins and pivots](11_data-manipulation.qmd) — assemble all extracts into one dataset
- [Phase 14d — Register reference](14d_register_reference.qmd) — column names for lpr_sksopr, procedurer_kirurgi etc.
