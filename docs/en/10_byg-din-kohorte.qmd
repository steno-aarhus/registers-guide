...
::: {.callout-note}
**Functions used here.**
`inner_join()` and `bind_rows()` are already shown in [Phase 9](09b_udtraek_fra_lpr.qmd) and explained in detail in [12b - Link your extracts](12b_joins.qmd#joins-link-two-tables). `group_by() + slice()` is explained in [12a - Long ↔ wide format](12a_langt-bredt.qmd). New in Step 2: `anti_join()`, `distinct()`, `pull()` plus `nrow()`, `cat()` and `stopifnot()` - see [Function guide](15a_guide_til_funktioner.qmd).
:::
...
`lpr_sksopr` holds the procedure code (`c_opr`) + `recnum`, but **not** `pnr` or date; those live in `lpr_adm`. So you join the two on `recnum` to get person + date + procedure together - exactly the same pattern as diagnoses (contact + diagnosis register), see [Phase 9a](09a_forstaa_lpr.qmd).
...
If the same check is needed more than once (e.g. also on a comparison group later, see [Phase 10a](10a_sammenligningskohorte.qmd)), wrap the logic in a function so the two checks do not drift apart. See [Good coding practice](12_god-kode-praksis.qmd).
