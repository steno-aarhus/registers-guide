---
title: "Want to help?"
subtitle: "Help expand the guide - share knowledge and code that can be reused across register-based research projects"
date: last-modified
---

This guide is continuously evolving, and many researchers have solved the same classic problems each in their own project - without the code or the experience ever being shared. If we share, we all save time.

There are **two kinds of contribution** we are especially looking for: **knowledge** (recurring problems, methods and packages we should cover) and **code** (concrete, reusable solutions). You do not need anything finished - drafts and good pointers are gold too.

---

## Share your knowledge

Where do register researchers keep hitting the same things? Point us in the right direction:

::: {.grid}

::: {.g-col-12 .g-col-md-4 style="border: 1px solid #dee2e6; border-radius: 8px; padding: 1.5rem; background: #eef3fb;"}
#### Recurring problems
Which pitfalls do you meet again and again? (immortal time, exclusion order, prevalent vs. incident, time-varying confounding ...) - so we can describe them in one place.
:::

::: {.g-col-12 .g-col-md-4 style="border: 1px solid #dee2e6; border-radius: 8px; padding: 1.5rem; background: #eef3fb;"}
#### Epidemiological methods
Designs, bias and analysis approaches specific to register data - and how to explain them simply for beginners.
:::

::: {.g-col-12 .g-col-md-4 style="border: 1px solid #dee2e6; border-radius: 8px; padding: 1.5rem; background: #eef3fb;"}
#### Packages that already exist
Do you know an R package that solves a register problem? (e.g. `heaven`, `Epi`, `MatchIt`, `EpiForsk`, `comorbidity` ...) Then we point to it instead of reinventing.
:::

:::

---

## Share your code

Concrete, reusable solutions. It does not need to be validated - we are happy to work on from a draft:

::: {.grid}

::: {.g-col-12 .g-col-md-4 style="border: 1px solid #dee2e6; border-radius: 8px; padding: 1.5rem; background: #fafafa;"}
#### Ethnicity
Deriving ethnicity/origin and generations from BEF/IEPE.
:::

::: {.g-col-12 .g-col-md-4 style="border: 1px solid #dee2e6; border-radius: 8px; padding: 1.5rem; background: #fafafa;"}
#### Medication and medication use
Exposure measures from LMDB: DDD, weighted use, PDC/MPR, insulin dosing, lag time and long-term prescriptions.
:::

::: {.g-col-12 .g-col-md-4 style="border: 1px solid #dee2e6; border-radius: 8px; padding: 1.5rem; background: #fafafa;"}
#### Family linkage
Linking parents, children and siblings via CPR/family id - e.g. to transfer exposures or covariates across generations.
:::

::: {.g-col-12 .g-col-md-4 style="border: 1px solid #dee2e6; border-radius: 8px; padding: 1.5rem; background: #fafafa;"}
#### Nordic Multimorbidity Index (NMI)
Computing NMI from LPR. We have a [starter page](14c_nmi.qmd) - validated code and variants are very welcome.
:::

::: {.g-col-12 .g-col-md-4 style="border: 1px solid #dee2e6; border-radius: 8px; padding: 1.5rem; background: #fafafa;"}
#### Socioeconomic classification
Education, income and employment, e.g. by SEPLINE. We have a [starter page](13_socioekonomiske-variable.qmd) - validated code welcome.
:::

::: {.g-col-12 .g-col-md-4 style="border: 1px solid #dee2e6; border-radius: 8px; padding: 1.5rem; background: #fafafa;"}
#### Time-varying variables
Variables that change over follow-up - blood tests, BMI, weight - in long format / intervals.
:::

::: {.g-col-12 .g-col-md-4 style="border: 1px solid #dee2e6; border-radius: 8px; padding: 1.5rem; background: #fafafa;"}
#### Simulated data
Simulated/synthetic datasets, so the guide's code examples can be run without access to real DST data.
:::

:::

---

::: {.callout-note}
It does not need to be finished or validated - we are happy to work further with drafts. If you have something else entirely that solves a general problem in register data, you are very welcome to get in touch as well.
:::

---

## How to contribute

Two ways - pick whichever suits you:

- **Email us (easiest):** [saras@clin.au.dk](mailto:saras@clin.au.dk). Send code, a draft, or just an idea - we'll add it. You need neither git nor GitHub.
- **Open a pull request on GitHub (if you're comfortable with it):** The guide's *source files* live in a GitHub repository: [github.com/saras-clin/DST_help](https://github.com/saras-clin/DST_help). Every page in the guide is a `.qmd` file, and the website itself is built automatically from them. A *pull request* is a proposal of concrete changes to those files that we can review and merge.

<details>
<summary>How do I send a pull request? (step by step)</summary>

**You** make the change and send the proposal; we review and merge it. Here's how:

1. **Create a (free) GitHub account** if you don't have one.
2. **Find the right file.** Every page in the guide is a `.qmd` file under `docs/da/` (Danish) and `docs/en/` (English) - e.g. this page is `docs/en/contribute.qmd`. To edit an existing page, open its `.qmd`; to add something new, create a new `.qmd` file. (The guide is bilingual, so please edit both languages - or write one and ask us for the other.)
3. **Edit.** For small fixes you can use the pencil icon on the file directly in the browser - no git needed. For larger contributions, make a **fork** (your own copy of the repo) and edit there.
4. **Open a pull request** back to `saras-clin/DST_help`. GitHub calls the button "Propose changes" / "Create pull request". It is a *proposal*, not a direct change.
5. We **review**, comment if needed, and **merge** it - and the website updates automatically.

In short: you don't need to know git to contribute (an email is perfectly fine), but if you like GitHub, a PR is the easiest way to get your code in - and you're credited as a contributor. See also [CONTRIBUTING.md](https://github.com/saras-clin/DST_help/blob/main/CONTRIBUTING.md) in the repo.

</details>
