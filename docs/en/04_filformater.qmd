...
The last point — **lazy vs. full loading** — is the most important distinction on this page. The mechanism behind it is explained in [Phase 5 — The universal pattern](05_universelle-moenster.qmd); here you just need to know *which* file types behave which way.
...
You write the path to the parquet folder containing the register — e.g. `open_dataset("path/to/bef/")`. The exact path depends on your project and server. Column names for each register are in [14d — Register reference](14d_register_reference.qmd).
...
**Never** save raw register data as CSV — only aggregated results. See [Phase 15 — Export and repatriation](15_eksport-hjemsendelse.qmd) for the rules.
...
→ [Phase 5 — The universal pattern](05_universelle-moenster.qmd)
