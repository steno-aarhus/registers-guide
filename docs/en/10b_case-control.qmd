...
The analysis itself belongs to [Phase 13 - Analysis](13_analyse.qmd) (not yet written) - here only briefly, so you know where this leads. Matched case-control data **can** be analyzed with **conditional logistic regression** (`survival::clogit()` with a `strata()` term per matched set), so the matching is respected; the odds ratio from an incidence-density design estimates the rate/hazard ratio. This is the standard approach for matched case-control studies (see e.g. Breslow & Day 1980, *The analysis of case-control studies*, IARC). Before the analysis you attach exposure and covariates to the matched persons (from [Phase 11](11_foej-variabler.qmd)/[Phase 12](11_data-manipulation.qmd)). The matched-set id is called `case.id` (Route A) or `Set` (Route B).
