...
::: {.callout-note}
**Functions used here.**
`left_join()` (attach a variable) and `transmute()` (create new columns, keep only those) are explained in [10a](10a_sammenligningskohorte.qmd) and [12b](12b_joins.qmd#joins-link-two-tables). `pmin()`, `if_else()` and `as.factor()` are in the [Function guide](15a_guide_til_funktioner.qmd). New here: `incidenceMatch()` / `Epi::ccwc()` for sampling and `survival::clogit()` for the analysis.
:::
...
*(Steps 1 and 2 - identify cases and exclude prevalent cases - are on the [landing page](10_byg-din-kohorte.qmd).)*

You need one row per person with matching variables and the dates eligibility depends on. Build it **exactly like the pool in [10a, Steps 3-4](10a_sammenligningskohorte.qmd)** (demographics + `earliest_bef` + death date + emigration date + lookback). For case-control, though:

- You do **not** need an exposure date as an eligibility criterion - exposure is a *variable* you measure, not a filter.
...
```r
# 'population' = one row per person from 10a (pnr, sex, birth_year,
#                death_date, emigration_date, earliest_bef_year). See 10a Steps 3-4.
...
**Route A - `heaven::incidenceMatch()`** (sibling of `exposureMatch()` from [10a](10a_sammenligningskohorte.qmd), but the "event" is the outcome).
...
<details>
<summary>Show code (Route B)</summary>
...
::: {.callout-note}
**Same rules as in [10a](10a_sammenligningskohorte.qmd):** apply inclusion/exclusion criteria BEFORE sampling and to everyone (cases + pool); set a `seed`; and decide on [with/without replacement](10a_sammenligningskohorte.qmd#with-or-without-replacement) (controls can typically be used for several cases) - which must be handled in the analysis. `incidenceMatch()` itself handles the risk-set time dimension via `case.index` + `end.followup`.
:::

---

## Step 5 - Analysis: conditional logistic regression {.step-heading}

Matched case-control data are analyzed with **conditional logistic regression** (`survival::clogit()`), so the matching is respected (each matched set is its own stratum). First attach exposure and covariates to the matched persons (from [Phase 11](11_foej-variabler.qmd)/[Phase 12](11_data-manipulation.qmd)).
...
# matched (from Route A) + exposure and covariates attached (Phase 11/12)
fit <- clogit(
  case ~ exposure + covariate1 + covariate2 + strata(case.id),  # strata() = respect the matched set
  data = matched_with_variables
)
summary(fit)        # exp(coef) = odds ratio, which here estimates the rate/hazard ratio
```

</details>

::: {.callout-note}
If you use `Epi::ccwc()` (Route B), the matched-set variable is called `Set`, so it is `strata(Set)`. The analysis step itself belongs to [Phase 13 - Analysis](13_analyse.qmd).
:::
...
| Extraction | Phase |
|---|---|
| Exposure + covariates | [Phase 11](11_foej-variabler.qmd) |
...
- [Build your study population](10_byg-din-kohorte.qmd) - identify your cases (Step 1) and exclude prevalent cases (Step 2)
