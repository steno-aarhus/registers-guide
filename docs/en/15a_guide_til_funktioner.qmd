...
What you write inside the parentheses is called the **arguments**. Many functions have *named* arguments, where the name tells you what the value controls, e.g. `ratio = 5` or `na.rm = TRUE`. You do not need to memorize them: `?functionname` shows which arguments a function takes and what they do.
...
**Analogy:** You have a column with date of birth.
You want a column with birth year.
`mutate(birth_year = year(foed_dag))` adds the new column without changing anything else.
The number of rows is the same after `mutate()` - it adds information, does not remove rows.

```r
mutate(icd3 = substr(c_diag, 2, 4))   # new column: ICD code without D-prefix, 3 characters
mutate(age = as.numeric(difftime(surgery_date, foed_dag, units = "days")) / 365.25)   # age at surgery in years
```
...
See [SEPLINE](13_socioekonomiske-variable.qmd).
:::
