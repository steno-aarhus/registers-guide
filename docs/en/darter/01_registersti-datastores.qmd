...
**Laboratory results - use only one source:** `laboratorieproevesvar_` (>2.2 billion rows) replaces `lab_forsker`/`lab_dm_forsker`. The old files still exist and cover the same data - use **only one** to avoid duplicates. Because the register is so large, `filter(pnr %in% !!kohort$pnr)` and `select()` **before** `collect()` are essential. Two things to watch when extracting:
...
- [Overview of registers](../15d_register_reference.qmd) - full confirmed column names
