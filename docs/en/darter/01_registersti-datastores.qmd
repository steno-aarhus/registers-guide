...
**Why it matters:** if you run on `dodsaars` only, everyone who dies after 2001 is treated as alive. That skews censoring and matching in `01_build_cohorts.R`. See [pitfall 1](02_darter-faldgruber.qmd#1-check-that-parquet-files-are-up-to-date).
...
See [pitfall 6 - Laboratory results](02_darter-faldgruber.qmd) for a code example.
...
::: {.callout-note}
**DARTER uses `dstDataPrep::load_database()`.** This is DARTER's internal connector, wired to project 708421's converted registers. On other projects the general-purpose, CRAN equivalent is `fastreg::read_register("name")` - see [Phase 4 - Convert SAS to parquet](../04_filformater.qmd#convert-sas-to-parquet). The two play the same role (resolve a register by name, return a lazy connection); `fastreg` is likely to replace `load_database` once the workspace is migrated to fastreg's layout.
:::
...
- [Overview of registers](../register_reference.qmd) - full confirmed column names
