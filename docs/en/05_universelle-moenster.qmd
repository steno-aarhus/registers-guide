...
Learn it here once, and you will recognise it everywhere.
...
## What is `!!` — and why is it necessary?
...
## RAM is shared — clean up after yourself
...
```{mermaid}
flowchart TD
    P[("Parquet files on DST")]
    L["open_dataset()"]
    F["filter() + select()"]
    C["collect()"]
    R[("R memory · data.frame")]

    P -->|"lazy connection — nothing moved to R"| L
    L -->|"instructions stacked up"| F
    F -->|"send now!"| C
    C -->|"only necessary rows + columns"| R
```
