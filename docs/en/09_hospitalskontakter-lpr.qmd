...
| | Approach 2 — direct extraction | Approach 1 — `alle_dx` |
|---|---|---|
| **Best when** | You have **one single outcome** | You have **multiple outcomes** from LPR |
...
<details>
<summary>Approach 2 — fetch specific diagnoses directly (start here for one outcome)</summary>
...
::: {.callout-note}
**Do you have F-codes (e.g. dementia, depression)?** Extend the regex to include them, e.g. `"^DE1[0-4]|^DF0[0-3]|^DG30"`, and add psychiatric LPR2 — see Approach 1 below for the code.
:::
...
<details>
<summary>Approach 1 — fetch all diagnoses + filter outcome (for multiple outcomes)</summary>
...
→ **Complete "Exclude prevalent cases" below, then continue with Part 2.**

---

**Part 2 — filter your specific outcome** *(requires `cohort_clean` from the Exclude step)*
...
outcome <- alle_dx %>%
  ...
  inner_join(cohort_clean %>% select(pnr, index_date), by = "pnr") %>%
