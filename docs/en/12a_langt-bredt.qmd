...
Instead use the pattern from [Phase 9](09b_udtraek_fra_lpr.qmd): extract the first date for one specific diagnosis group (with `group_by() + slice()` above), save as `extract_dementia.rds`, and join it as one column with `left_join()`.
:::
...
→ [12b - Link your extracts](12b_joins.qmd)

## See also

- [12b - Link your extracts](12b_joins.qmd) - joins, `bind_rows()` and calculating follow-up time
