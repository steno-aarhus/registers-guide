...
subtitle: "group_by() + slice(), joins and bind_rows() — assemble your extracts into one dataset"
date: last-modified
---

Your extracts from [Phase 6](06_foerste-udtraek.qmd), [Phase 9](09b_udtraek_fra_lpr.qmd) and [Phase 11](11_foej-variabler.qmd) are stored as separate RDS files — some with one row per person, others with many. Here you reduce each extract to **one row per person** and join them on `pnr` into one analysis-ready dataset. Finally, you calculate follow-up time and the event variable.
...
`left_join()` behaves predictably when both tables have **one row per `pnr`**. Many register extracts have multiple rows per person (long format — see [12b — Long ↔ wide format](12b_pivots.qmd)). `group_by() + slice()` is the primary way to reduce a dataset to one representative row per person before joining — **it is preparation for the join, not the join itself**.
...
If you want **two or more time points side by side** as separate columns in the analysis — use `pivot_wider()` ([12b](12b_pivots.qmd)).
...
- [12b — Long ↔ wide format](12b_pivots.qmd) — reshape a table with `pivot_wider()` / `pivot_longer()`
