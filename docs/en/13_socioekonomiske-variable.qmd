...
::: {.callout-warning}
**Fetch the variable for the year BEFORE index - not the index year itself.** All three registers are **annual**: each person has one value per calendar year, and it typically describes status at **year-end** (e.g. income totalled for the whole year, education as of 30 September).

If you take the value from the *index year*, you risk measuring status that falls **after** your exposure - i.e. after index. Example: index = surgery date 15 June 2015. The 2015 value reflects status late in 2015, i.e. *after* the surgery, and may be affected by the exposure itself (e.g. job loss after illness). That gives reverse causation - you want a **baseline** value from before the exposure.

So you fetch the value for `year(index_date) - 1` (here 2014). This is a safe, uniform rule for the whole cohort, regardless of when in the year index falls. If you want status closer to baseline for people with an index early in the year, you could consider a finer rule, but the year-before is the standard choice.
:::
...
- [Format tables](15b_formateringstabeller.qmd) - DST's SAS files for label translation
- [Overview of registers](15d_register_reference.qmd) - confirmed column names for AKM, FAIK and UDDA
...
- **Specialist packages** (OSDC for diabetes classification, NMI for comorbidity score, analysis tools)? → [Algorithms & special packages](14a_pakker-oversigt.qmd)
- **Ready to export results?** → [Phase 14 - Export and repatriation](16_eksport-hjemsendelse.qmd)
