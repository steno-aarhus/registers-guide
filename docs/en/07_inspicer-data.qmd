...
An overview of common error messages and what they typically mean is in [Phase 15c — DST pitfalls](15c_dst_faldgruber.qmd#the-most-common-error-messages-and-what-they-mean).
...
Does the object look like data but behave strangely, or does your code fail with a mysterious message? Run `class(your_object)` — if it is not `"data.frame"`, you are probably missing a `collect()`. The full table of what `class()` can return — and why — is in [Phase 5 — Extracting data step by step](05_universelle-moenster.qmd#check-do-you-have-data-or-just-a-connection).
:::
...
See [Phase 15c — Pitfalls](15c_dst_faldgruber.qmd#4-date-columns-are-not-always-in-date-format) for the most common date conversion errors and how to fix them.
...
- [Phase 8 — Know your registers](08_kend-dine-registre.qmd) — decision table: which register contains what
- [Phase 15a — Functions: overview](15a_guide_til_funktioner.qmd) — `filter()`, `select()`, `mutate()`, `left_join()` and more
- [`dataReporter`](14a_pakker-oversigt.qmd#datareporter-automatic-data-quality-report) — automatically generates an HTML report of all columns (distribution, missingness, outliers). Confirmed on DST.
