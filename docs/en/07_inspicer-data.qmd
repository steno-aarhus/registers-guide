...
An overview of common error messages and what they typically mean is in [Phase 14c — DST pitfalls](14c_dst_faldgruber.qmd#the-most-common-error-messages-and-what-they-mean).
...
Does the object look like data but behave strangely, or does your code fail with a mysterious message? Run `class(your_object)` — if it is not `"data.frame"`, you are probably missing a `collect()`. The full table of what `class()` can return — and why — is in [Phase 5 — The universal pattern](05_universelle-moenster.qmd#check-do-you-have-data-or-just-a-connection).
:::
...
See [Phase 14c — Pitfalls](14c_dst_faldgruber.qmd#4-date-columns-are-not-always-in-date-format) for the most common date conversion errors and how to fix them.
...
- [Phase 14a — Functions: overview](14a_guide_til_funktioner.qmd) — `filter()`, `select()`, `mutate()`, `left_join()` and more
- [`dataReporter`](13a_pakker-oversigt.qmd#datareporter-automatic-data-quality-report) — automatically generates an HTML report of all columns (distribution, missingness, outliers). Confirmed on DST.
