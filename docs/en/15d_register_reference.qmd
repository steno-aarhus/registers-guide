...
### lpr_sksube — SKS examinations (ZZ codes)

Examination/sub-procedure codes (ZZ codes) live in `lpr_sksube`, separate from the operation codes above. Join like the other SKS tables — via `recnum` (LPR2). Less commonly used, and column names are not verified here — look them up in [DST's variable list](https://www.dst.dk/extranet/forskningvariabellister/Oversigt%20over%20registre.html) and confirm with `colnames()`.
