...
The code examples on this page use `open_dataset("path/to/register/")` as a generic placeholder — replace with the path to your project's parquet folder.
...
See [Pitfalls](15c_dst_faldgruber.qmd) for special quirks with each register.
Column names apply after `rename_with(tolower)` — inspect your own register with `colnames()` if something does not match.

::: {.callout-tip}
**Looking for "which register contains X?"** — start with the decision table in [Phase 8 — Know your registers](08_kend-dine-registre.qmd). This page is the deep reference with full column names, types and code examples.
:::
...
| Laboratory results | `"lab_forsker"` | `pnr` | Varies | NPU codes, test results — section 6 |
...
### Laboratory results — lab_forsker

Laboratory/blood test results (e.g. HbA1c, lipids). A **very large** register. Central fields: `pnr`, NPU code (analyte type), result value (often character — e.g. "not detected"), unit and sample date. Filter and `select()` **before** `collect()`. Availability and structure are highly project-/DARTER-specific.
