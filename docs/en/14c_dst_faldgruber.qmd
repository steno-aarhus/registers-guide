...
See [The universal pattern](05_universelle-moenster.qmd) for explanation and example.
...
on lazy DuckDB connections. See [Functions guide](14a_guide_til_funktioner.qmd#bang-bang-two-exclamation-marks) for full explanation.
:::
...
|---|---|---|
| `nmi_score` | **Weighted** comorbidity score — Nordic Multimorbidity Index (Kristensen et al., *Clin Epidemiol* 2022). 50 predictors with individual weights; lung cancer counts e.g. 19 points, type 2 diabetes counts 2. | See [NMI page](13c_nmi.qmd) |
...
| `Error in filter.default(...)` | Filtering on a lazy object without `%>%` | Switch to `%>%` — see [the pipe](14a_guide_til_funktioner.qmd#the-pipe) |
