...
collect(select(semi_join(lpr_adm, tibble(pnr = my_pnrs), by = "pnr"), pnr, recnum))
...
`read_register()` finds the path from your project config (set once) - see [Phase 4](04_filformater.qmd#convert-sas-to-parquet). It returns a DuckDB connection.
...
The confirmed paths for your project are in [Overview of registers](register_reference.qmd).
...
You do not need this step with `load_database()` (dstDataPrep) or `read_register()` (fastreg) - they hand you a DuckDB connection already, so the `to_duckdb()` conversion is built in. For when to use which, see [Phase 5 - Arrow vs. DuckDB](05_udtraek-trin-for-trin.qmd#arrow-vs-duckdb).
:::
...
::: {.callout-note}
The concept of lazy evaluation - why data is not in memory before `collect()` - is explained in detail on the page [Extracting data step by step](05_udtraek-trin-for-trin.qmd).
:::
...
See [File types](04_filformater.qmd) and [Format tables](formateringstabeller.qmd) for practical examples.
...
If you forget it, `semi_join(..., by = "pnr")` will fail because the column might be called `PNR`.
...
`icd3 %in% c("G30", "F00", "F03")` is like standing at the entrance and checking: "is this diagnosis code on the list?"
...
**Analogy:** Imagine asking an assistant to find all rows with a code from a list.
If you say "find all with a code from the list `my_atc`", the assistant will look for a column in the database with that name - and it does not exist.
...
- **Age at index** (`/365.25`, leap years): [Comparison cohort](10a_sammenligningskohorte.qmd)
- **Baseline year for annual registers** (`year(index_date) - 1`): [Socioeconomic variables](11b_socioekonomiske-variable.qmd)
- **SAS integer dates** (`origin = "1960-01-01"`): [Pitfalls on DST](dst_faldgruber.qmd)
- **Follow-up time and event variable** (`pmin()`): [Joins](12b_joins.qmd#opfoelgningstid)
- **Start/stop format** (time-varying covariate): [Time-varying variables](12d_tidsvarierende.qmd)
:::
...
See [Overview of registers](register_reference.qmd) for confirmed column names on all registers these packages work against.

| Package | What it provides |
|---|---|
| `dstDataPrep` | `load_database()` - DST-specific parquet connector *(DARTER project only; the general-purpose CRAN equivalent is `fastreg::read_register()`)* |
...
- [Writing functions](https://www.epirhandbook.com/en/new_pages/writing_functions.html)
- [Iteration, loops and lists](https://www.epirhandbook.com/en/new_pages/iteration.html)
:::
