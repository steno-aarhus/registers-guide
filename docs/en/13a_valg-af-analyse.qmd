...
**Paired or unpaired?** Data are *paired* when the measurements go together two by two - typically the **same person before and after** a treatment, or a **1:1 matched pair** (one case and one matched control). The effect is then computed *within* each pair, and you use a paired test (e.g. paired t-test or McNemar). Data are *unpaired* when the groups are **different, independent people**.

**Watch out: matched with several controls is not "paired".** 1:1 matching is paired, but if you have matched **1:n** (e.g. 1:5 case-control) or a **matched cohort**, there are matched sets (strata) rather than pairs - and it is not a paired test. Use instead **conditional logistic regression** (`clogit` with `strata()`, see [Regression](13d_regression.qmd)) for matched case-control, or **stratified/clustered Cox** for a matched cohort (see [Time-to-event](13e_time-to-event.qmd)). So they belong under regression / time-to-event in the tables below.
