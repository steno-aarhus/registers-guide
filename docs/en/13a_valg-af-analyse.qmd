...
**Parametric or nonparametric?** A *parametric* test or model assumes the data follow a particular distribution (usually the normal, or e.g. a binomial assumption in a regression model) and works with e.g. means and standard deviations. A *nonparametric* test assumes no particular distribution of the underlying values - it works on the **ranks** of the values (for numbers, e.g. Wilcoxon) or on **counts in categories** (for categorical data, e.g. chi-square), so it is more robust to skewed distributions and outliers, but usually has slightly less power. Rule of thumb: if the parametric test's assumptions hold (see [Assumptions](#assumptions) below), use it; otherwise the nonparametric one.
...
<td>Nonparametric</td><td>Binomial test</td><td>Binomial distribution; independent</td><td><code>binom.test(x, n, p = p0)</code> (exact); <code>prop.test(x, n, p = p0)</code> (approx.)</td>
</tr>
<tr>
<td rowspan="3">Proportion, two groups</td>
<td rowspan="2">Unpaired</td>
<td>Compare two unpaired groups</td>
<td>Nonparametric</td><td>Chi-square / Fisher's exact</td><td>Independent (Fisher for small counts)</td><td><code>chisq.test(table)</code>; <code>fisher.test(table)</code></td>
</tr>
<tr>
...
<td>Nonparametric</td><td>McNemar</td><td>Paired obs.; independent pairs</td><td><code>mcnemar.test(table)</code></td>
</tr>
<tr>
<td>Regression</td>
<td>Unpaired</td>
