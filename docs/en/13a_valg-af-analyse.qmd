...
Which analysis you need depends mainly on the **data type** (numeric, binary or time-to-event), whether the data are **paired or unpaired**, and whether the assumptions for a **parametric** test hold (otherwise a nonparametric one).

**Paired or unpaired?** Data are *paired* when the two measurements go together in pairs - e.g. the **same person before and after** a treatment, or **matched pairs** (a case and a control matched 1:1). Data are *unpaired* when the groups are **different, independent people**. Paired data are analysed with paired tests, because each difference is computed within a pair. The tables below match each situation to a test/model and the corresponding **R** command.
