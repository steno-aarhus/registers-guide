...
**Start with your question.** First state your research question and **null hypothesis** - they drive everything else. Then ask: do you want to **compare groups** or **describe a relationship**? (These are fundamentally the same - comparing groups is examining a relationship between a grouping variable and the outcome.)

Which test you need then depends mainly on the **data type** (numeric, binary or time-to-event), whether the data are **paired or unpaired**, and whether the assumptions for a **parametric** test hold (otherwise a nonparametric one). The tables below match each situation to a test/model and the corresponding **R** command.
...
**Note: matched with several controls is not treated as paired.** 1:1 matching corresponds to paired data, but if you match with **several controls** (1:n, e.g. 1:5 case-control) or have a **matched cohort**, there are matched sets (strata) with more than two members each - so a paired test, which needs pairs, does not fit. Instead it is analysed with **conditional logistic regression** (`clogit` with `strata()`, see [Regression](13d_regression.qmd)) for matched case-control, or **stratified/clustered Cox** for a matched cohort (see [Time-to-event](13e_time-to-event.qmd)) - methods that generalise the pairing idea to matched sets of any size. So they belong under regression / time-to-event in the tables below.

**Parametric or nonparametric?** A *parametric* test or model assumes the data follow a particular distribution - usually the normal for tests on numbers, or the binomial for tests on proportions. A *nonparametric* test assumes no particular distribution and instead works on the **ranks** of the values (e.g. Wilcoxon); it is more robust to skewed distributions and outliers, but usually has slightly less power. Rule of thumb: if the parametric test's assumptions hold (see [Assumptions](#assumptions) below), use it; otherwise the nonparametric one.
...
<details>
<summary>Independent vs. paired data</summary>

- **Independent data:** different, unrelated people in each group (e.g. one group gets diet 1, another diet 2). Recording e.g. sex on each person does not make it paired - sex and outcome are just two different variables.
- **Paired data:** the **same** unit is measured more than once (e.g. weight before and after a treatment) or forms 1:1 matched pairs. This decides whether you use a paired or unpaired test (see the tables).
