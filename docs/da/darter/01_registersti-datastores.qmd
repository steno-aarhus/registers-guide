...
::: {.callout-note}
**DARTER bruger `dstDataPrep::load_database()`.** Det er DARTERs interne connector, koblet til projekt 708421's konverterede registre. På andre projekter er den generelle CRAN-ækvivalent `fastreg::read_register("navn")` - se [Fase 4 - Konvertér SAS til parquet](../04_filformater.qmd#konvertér-sas-til-parquet). De to spiller samme rolle (slå et register op via navn, returnér en doven forbindelse); `fastreg` afløser sandsynligvis `load_database`, når workspacet er migreret til fastregs layout.
:::
