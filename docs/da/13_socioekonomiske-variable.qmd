...
::: {.callout-warning}
**Hent variablen for året INDEN index - ikke index-året selv.** Alle tre registre er **årsbaserede**: hver person har én værdi per kalenderår, og den beskriver typisk status ved **årets udgang** (fx indkomst opgjort for hele året, uddannelse pr. 30. september).

Hvis du tager værdien fra *index-året*, risikerer du at måle status, der ligger **efter** din eksponering - altså efter index. Eksempel: index = operationsdato 15. juni 2015. 2015-værdien afspejler status sent i 2015, dvs. *efter* operationen, og kan være påvirket af selve eksponeringen (fx tab af job efter sygdom). Det giver omvendt kausalitet - du vil måle en **baseline**-værdi, der ligger før eksponeringen.

Derfor henter du værdien for `year(index_date) - 1` (her 2014). Det er en sikker, ens regel for hele kohorten, uanset hvornår på året index falder. Vil du have status tættere på baseline for personer med index tidligt på året, kan du overveje en mere finkornet regel, men år-inden er standardvalget.
:::
...
- **Specialpakker** (OSDC til diabetesklassifikation, NMI til comorbiditetsscore, analyseværktøjer)? → [Algoritmer & specialpakker](14a_pakker-oversigt.qmd)
- **Klar til at eksportere resultater?** → [Fase 14 - Eksport og hjemsendelse](16_eksport-hjemsendelse.qmd)
