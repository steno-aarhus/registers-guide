...
`anti_join()` (behold rækker *uden* match) er ny her — se forklaringen i kodeblokken og i [Fase 12](11_data-manipulation.qmd#joins-kobl-to-tabeller-sammen). `inner_join()`, `bind_rows()` og `group_by() + slice()` er vist i [Fase 9](09b_udtraek_fra_lpr.qmd) og [Fase 12](11_data-manipulation.qmd).
:::

::: {.callout-tip}
**Lidt teori, før du matcher.** Tag eksplicit stilling til:

- **Matchvariable:** typisk køn, fødselsår og kalenderår; ofte også SES og kommune. Match kun på det du vil justere for ved design.
- **Risk-set / incidens-densitets-matching:** en sammenligningsperson skal være i risiko **på den tildelte index-dato** — ikke kun ved studiestart. En person kan være komparator nu og selv blive eksponeret (case) senere.
- **Varierende vs. fast index-dato:** når index-dato varierer per eksponeret (fx hver ny T2D-patient får sin egen dato), skal komparatoren matches på *netop den dato* — det er præcis hvad risk-set sampling gør.
- **Med eller uden replacement:** må en person være komparator for flere eksponerede? Replacement øger den effektive stikprøvestørrelse, men giver korrelerede observationer, som skal håndteres i analysen.
:::
...
| Komorbiditet (NMI, Charlson) | [NMI](14c_nmi.qmd) |
