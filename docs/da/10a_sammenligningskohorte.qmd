...
`left_join()` (tilknyt en variabel) og `semi_join()` (behold kun rækker *med* match) forklares i [Kobl udtræk sammen](12b_joins.qmd#joins-kobl-to-tabeller-sammen). `mutate()` (lav en ny variabel) er vist i [Fase 2](02_r-intro.qmd). Nyt her: en `for`-løkke + `set.seed()` til selve matchingen (forklares i Trin 5). Funktioner som `slice_sample()`, `count()` og `nrow()` er i [Guide til funktioner](guide_til_funktioner.qmd).
:::
...
*(Trin 1 og 2 - identificér de eksponerede og ekskludér prævalente tilfælde - ligger på [forsiden](10_byg-studiepopulation.qmd).)*
...
::: {.callout-note}
**Lav en afledt variabel.** En ny variabel beregnes fra eksisterende kolonner med `mutate()` (uddybet i [Funktioner: oversigt](guide_til_funktioner.qmd)). Alder ved index = antal dage mellem index og fødsel, delt med 365.25 (et år er 365.25 dage pga. skudår):

```r
mutate(age_at_index = as.numeric(index_date - foed_dag) / 365.25)
```

Samme princip for andre afledte mål, fx BMI fra højde og vægt: `mutate(bmi = vaegt_kg / (hoejde_m^2))`. Beregn variablen ÉN gang og genbrug den, så match-alder og Table 1 altid er enige. Se [Fase 12](12_data-manipulation.qmd#afledte-variable-lav-nye-kolonner) for flere afledte variable (kategorisering, hændelses-indikator).
:::
...
Poolen bygger vi **oven på `demografi` og `earliest_bef` fra Trin 3** (samme BEF-udtræk - vi henter det altså ikke igen) og tilføjer alt, som eligibilitet afhænger af: **dødsdato**, **udvandringsdato**, **eksponeringsdato**, **outcome-dato** (så outcome før den tildelte index-dato kan ekskluderes for sammenligningspersonen, præcis som for de eksponerede i Trin 3) og **lookback-år**. Datoerne udtrækkes direkte for befolkningen med samme mønstre som [Udtræk fra LPR](09b_udtraek_fra_lpr.qmd) (her vist som færdige tabeller).
...
Hver dato-tabel ender som **én række per person** (kun den dato, eligibilitet afhænger af) - derfor kobler pool-joinet nedenfor på `pnr`. Udtrækket er **samme slags som i [Fase 9](09b_udtraek_fra_lpr.qmd)**; eneste forskel er, at du **ikke** begrænser til de eksponeredes `pnr`: du beholder alle, fordi en sammenligningsperson kan være hvem som helst.

**Vigtigt om `diagnoser`:** det er den **samlede, harmoniserede** LPR-diagnosetabel fra Fase 9 - ikke en rå tabel. Den bygges der ved at joine `lpr_adm`+`lpr_diag` på `recnum` (LPR2) og `lpr_a_kontakt`+`lpr_a_diagnose` på `dw_ek_kontakt` (LPR3), kombinere psykiatri og somatik, og harmonisere datokolonnen til `date_contact` (omdøbt fra `d_inddto` hhv. `kont_starttidspunkt`). For hele befolkningen kører du nøjagtig det [9b-udtræk](09b_udtraek_fra_lpr.qmd), bare **uden** `filter(pnr %in% kohort_pnrs)`. De egentlige join-nøgler ligger altså i 9b (`recnum`/`dw_ek_kontakt`); her i pool-joinet er nøglen `pnr`, fordi tabellerne nu er én række per person.
...
# Samme mønstre som i Fase 12 - bemærk: INGEN filter(pnr %in% eksponerede$pnr).
...
# Gem dem som .rds, så de kan genbruges senere - præcis de filer, kodeblokken nedenfor kan indlæse igen:
...
| Udtræk | Fase |
|---|---|
| Udfald (diagnoser, dødsdato, emigration) | [Udtræk fra LPR](09b_udtraek_fra_lpr.qmd), [Udfald](11a_udfald.qmd) |
...
- [Byg din studiepopulation](10_byg-studiepopulation.qmd) - identificér de eksponerede (Trin 1) og ekskludér prævalente (Trin 2)
...
- [Fase 12 - Saml & klargør datasættet](12_data-manipulation.qmd) - saml alle udtræk til ét datasæt
