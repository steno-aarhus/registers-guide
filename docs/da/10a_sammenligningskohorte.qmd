...
Poolen bygger vi **oven på `demografi` og `earliest_bef` fra Trin 3** (samme BEF-udtræk - vi henter det altså ikke igen) og tilføjer alt, som eligibilitet afhænger af: **dødsdato**, **udvandringsdato**, **eksponeringsdato**, **outcome-dato** (så outcome før den tildelte index-dato kan ekskluderes for sammenligningspersonen, præcis som for de eksponerede i Trin 3) og **lookback-år**. Datoerne udtrækkes direkte for befolkningen med samme mønstre som [9b: Udtræk fra LPR](09b_udtraek_fra_lpr.qmd) (her vist som færdige tabeller).
...
**Eneste forskel fra 9b:** du udelader `filter(pnr %in% kohort_pnrs)`, så du beholder HELE befolkningen - ikke kun de eksponerede. (Join-nøglerne `recnum`/`dw_ek_kontakt` hører altså til selve LPR-udtrækket i 9b; her i pool-joinet er nøglen `pnr`, fordi tabellerne nu er én række per person.)
