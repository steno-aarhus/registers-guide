...
Det er **samme slags udtræk som i Trin 2 / [Fase 9](09b_udtraek_fra_lpr.qmd)** - eneste forskel er, at du **ikke** begrænser til de eksponeredes `pnr`: du beholder alle, fordi en sammenligningsperson kan være hvem som helst. Hver tabel ender som én række per person (kun den dato, eligibilitet afhænger af).

```r
library(arrow); library(dplyr)

# --- Første outcome-dato for ALLE ---
# 'diagnoser' er dit LPR-diagnoseudtræk (pnr, date_contact, icd3) fra Fase 9.
# Henter du på outcome-koderne, får du ALLE personer med diagnosen - uanset eksponering.
# (I Trin 2 joinede vi til de eksponerede; her aggregerer vi bare for alle.)
udfald <- diagnoser %>%
  filter(icd3 %in% OUTCOME_KODER) %>%                # samme outcome-koder som i Trin 2
  group_by(pnr) %>%
  summarise(first_outcome_date = min(date_contact), .groups = "drop")  # tidligste outcome per person
...
eksp_dt <- eksponering_raw %>%                        # dit eksponerings-udtræk, én række per eksponeringshændelse
  group_by(pnr) %>%
  summarise(exposure_date = min(exposure_date), .groups = "drop")      # første eksponering per person
