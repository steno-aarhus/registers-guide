...
`left_join()` (tilknyt en variabel) og `transmute()` (lav nye kolonner, behold kun dem) forklares i [Sammenligningskohorte](10a_sammenligningskohorte.qmd) og [12b](12b_joins.qmd#joins-kobl-to-tabeller-sammen). `pmin()`, `if_else()` og `as.factor()` er i [Guide til funktioner](15a_guide_til_funktioner.qmd). Nyt her: `incidenceMatch()` / `Epi::ccwc()` til sampling og `survival::clogit()` til analysen.
:::
...
Selve analysen hører til [Fase 13 - Analyse](13_analyse.qmd) (endnu ikke skrevet) - her blot kort, så du ved, hvor det bærer hen. Matchede case-control-data **kan** analyseres med **betinget logistisk regression** (`survival::clogit()` med et `strata()`-led per matchsæt), så matchningen respekteres; odds ratio fra et incidens-densitets-design estimerer rate-/hazard ratio. Det er standardtilgangen for matchede case-control-studier (se fx Breslow & Day 1980, *The analysis of case-control studies*, IARC). Inden analysen tilføjer du eksponering og covariater til de matchede personer (fra [Fase 11](11_foej-variabler.qmd)/[Fase 12](11_data-manipulation.qmd)). Matchsæt-id'et hedder `case.id` (Vej A) eller `Set` (Vej B).
