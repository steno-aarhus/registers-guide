...
`left_join()` (tilknyt en variabel) og `transmute()` (lav nye kolonner, behold kun dem) forklares i [10a](10a_sammenligningskohorte.qmd) og [12b](12b_joins.qmd#joins-kobl-to-tabeller-sammen). `pmin()`, `if_else()` og `as.factor()` er i [Guide til funktioner](15a_guide_til_funktioner.qmd). Nyt her: `incidenceMatch()` / `Epi::ccwc()` til sampling og `survival::clogit()` til analysen.
:::
...
*(Trin 1 og 2 - identificér cases og ekskludér prævalente tilfælde - ligger på [forsiden](10_byg-din-kohorte.qmd).)*

Du skal bruge én række per person med matchvariable og de datoer, eligibilitet afhænger af. Byg den **præcis som poolen i [10a, Trin 3-4](10a_sammenligningskohorte.qmd)** (demografi + `earliest_bef` + dødsdato + udvandringsdato + lookback). Til case-control gælder dog:

- Du behøver **ikke** eksponeringsdato som eligibilitetskrav - eksponering er en *variabel*, du måler, ikke et filter.
...
```r
# 'befolkning' = én række per person fra 10a (pnr, sex, birth_year,
#                death_date, emigration_date, earliest_bef_year). Se 10a Trin 3-4.
...
**Vej A - `heaven::incidenceMatch()`** (søster til `exposureMatch()` fra [10a](10a_sammenligningskohorte.qmd), men "event" er udfaldet).
...
::: {.callout-note}
**Samme regler som i [10a](10a_sammenligningskohorte.qmd):** anvend inklusions-/eksklusionskriterier FØR sampling og på alle (cases + pool); sæt en `seed`; og tag stilling til [med/uden replacement](10a_sammenligningskohorte.qmd#med-eller-uden-replacement) (kontroller kan typisk bruges til flere cases) - det skal håndteres i analysen. `incidenceMatch()` håndterer selv risk-set-tidsdimensionen via `case.index` + `end.followup`.
:::

---

## Trin 5 - Analyse: betinget logistisk regression {.step-heading}

Matchede case-control-data analyseres med **betinget logistisk regression** (`survival::clogit()`), så matchningen respekteres (hvert matchsæt er sit eget stratum). Tilføj først eksponering og covariater til de matchede personer (fra [Fase 11](11_foej-variabler.qmd)/[Fase 12](11_data-manipulation.qmd)).
...
# matched (fra Vej A) + eksponering og covariater tilknyttet (Fase 11/12)
fit <- clogit(
  case ~ eksponering + covariat1 + covariat2 + strata(case.id),  # strata() = respektér matchsættet
  data = matched_med_variable
)
summary(fit)        # exp(coef) = odds ratio, der her estimerer rate-/hazard ratio
```

</details>

::: {.callout-note}
Bruger du `Epi::ccwc()` (Vej B), hedder matchsæt-variablen `Set`, så det er `strata(Set)`. Selve analysetrinnet hører til [Fase 13 - Analyse](13_analyse.qmd).
:::
...
- [Byg din studiepopulation](10_byg-din-kohorte.qmd) - identificér dine cases (Trin 1) og ekskludér prævalente (Trin 2)
