...
`left_join()` (tilknyt en variabel) og `transmute()` (lav nye kolonner, behold kun dem) forklares i [Sammenligningskohorte](10a_sammenligningskohorte.qmd) og [Kobl udtræk sammen](12b_joins.qmd#joins-kobl-to-tabeller-sammen). `pmin()`, `if_else()` og `as.factor()` er i [Guide til funktioner](15a_guide_til_funktioner.qmd). Nyt her: `incidenceMatch()` / `Epi::ccwc()` til sampling og `survival::clogit()` til analysen.
:::
