...
Denne side viser, hvordan du trækker diagnoser ud af LPR med kode. Den bygger på strukturen fra [Fase 9a — Forstå LPR](09a_forstaa_lpr.qmd): perioderne (LPR2/LPR3), D-præfikset, diagnosetyperne (A/B/G) og filtret for tilbagekaldte diagnoser. Læs den side først, hvis du ikke allerede har.
...
For at bygge din kohorte (Fase 10) skal du vide, hvordan man udtrækker diagnosekoder og procedurer fra LPR. For at udtrække diagnosekoder fra LPR (denne side) har du brug for en kohorte. Vi løser det sådan: **denne side lærer dig udtræksmønstret** — vi antager at du allerede har en kohorte. **[Fase 10](10_byg-din-kohorte.qmd) viser dig, hvordan du bygger den kohorte** ved at bruge præcis det mønster du lige har lært. Læs faserne i rækkefølge, og kom tilbage til koden her, når du har din kohorte klar.
:::

::: {.callout-warning}
**LPR_A-variabelnavne er nye — verificér mod dine egne data.** LPR3 er for nylig skiftet fra det gamle LPR_F-format til det nye LPR_A-format (se [LPR3 findes i to filformater](#lpr-a-format) nedenfor), og det skift har ændret en række variabelnavne — ofte uigennemsigtige danske forkortelser. Kodeeksemplerne bruger de navne registrene typisk har, men tjek altid med `colnames(din_data)` mod de faktiske filer, og slå dem op i [Fase 15 — Registerreference](15d_register_reference.qmd), før du stoler på et kolonnenavn.
:::
...
**LPR3 findes i to filformater — vær opmærksom på hvilke filer du loader**
DST har skiftet LPR3 fra det gamle **LPR_F**-format (filer som `kontakter`, `diagnoser`, `forloeb`) til det nye **LPR_A**-format (`lpr_a_kontakt`, `lpr_a_diagnose`). Afhængigt af hvordan dit projekts data er leveret, kan **begge versioner ligge i din datamappe** og dække de samme år. Loader du begge — eller blander gammelt og nyt — får du **duplikerede rækker**.

Brug konsekvent **LPR_A**-filerne (`lpr_a_*`), som i eksemplerne her, og lad de gamle LPR_F-filer ligge.

Nogle projekter har desuden ældre data *inde i* `lpr_a_kontakt`, som **allerede findes i LPR2** — kombinerer du de to perioder uden at filtrere, tælles de samme kontakter dobbelt. Filtrér dem fra med `lprindberetningssystem == "LPR3"`. **Det er projekt-specifikt** — er du på **DARTER**, er det håndteret i [faldgrube 5](darter/02_darter-faldgruber.qmd#lpr3-lprindberetningssystem). Ellers: afklar med din datamanager, om dit projekt kræver et tilsvarende filter.
:::
...
- [Fase 9a — Forstå LPR](09a_forstaa_lpr.qmd) — struktur, perioder, D-præfiks og diagnosetyper
- [Fase 6 — Første udtræk](06_foerste-udtraek.qmd) — trin-for-trin introduktion til open_dataset, collect og saveRDS
- [Fase 15 — Registerreference](15d_register_reference.qmd) — bekræftede kolonnenavne for alle LPR-registre
- [Fase 15 — Faldgruber](15c_dst_faldgruber.qmd) — kendte problemer med LPR på DST
