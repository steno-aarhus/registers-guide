...
- **Verificér kolonnenavne.** Skiftet til LPR_A har ændret en række variabelnavne (ofte uigennemsigtige danske forkortelser). Eksemplerne bruger de navne registrene typisk har, men tjek altid med `colnames(din_data)` og slå op i [Register-overblik](register_reference.qmd), før du stoler på et kolonnenavn.
:::
...
lpr2_dm <- lpr_adm %>%
  filter(pnr %in% !!kohort_pnrs) %>%                 # KUN din kohorte. UDELAD linjen, hvis du vil have HELE befolkningen
...
lpr3_dm <- lpr3_k %>%                                 # samme mønster som LPR2 - men LPR3-tabeller og andre kolonnenavne
  filter(pnr %in% !!kohort_pnrs) %>%                  # KUN din kohorte. UDELAD linjen for HELE befolkningen
