...
::: {.callout-warning}
**LPR_A-variabelnavne er nye - verificér mod dine egne data.** LPR3 er for nylig skiftet fra det gamle LPR_F-format til det nye LPR_A-format (se [LPR3 findes i to filformater](#lpr-a-format) nedenfor), og det skift har ændret en række variabelnavne - ofte uigennemsigtige danske forkortelser. Kodeeksemplerne bruger de navne registrene typisk har, men tjek altid med `colnames(din_data)` mod de faktiske filer, og slå dem op i [Register-overblik](15d_register_reference.qmd), før du stoler på et kolonnenavn.
:::

::: {.callout-note}
Koden bruger `inner_join()` og `bind_rows()`. Er det nye begreber, er de forklaret grundigt i [Fase 12 - Saml & klargør datasættet](11_data-manipulation.qmd) - du kan sagtens følge koden her og vende tilbage til Fase 12 bagefter.
:::
