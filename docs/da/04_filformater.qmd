...
Den nemmeste måde at konvertere på er pakken [`fastreg`](https://dp-next.github.io/fastreg/) (dp-next, på CRAN). Den konverterer SAS-registre til Parquet, partitionerer dem efter år og læser dem ind igen som dovne forbindelser - alt sammen via registrets **navn**, så du aldrig skal skrive lange stier igen. Den kan også køre konverteringen parallelt på flere "cores".

```r
install.packages("fastreg")   # én gang - fastreg er på CRAN

library(tidyverse)   # generel databehandling (dplyr, purrr, …)
library(furrr)       # kør konverteringen på flere cores
library(fastreg)

# 1. Sæt placeringen af rådata og output ÉN gang, øverst i scriptet.
#    Derefter refererer du til registre via navn - ingen fuld sti i hvert kald.
options(
...
rawdata_dir  <- getOption("fastreg.project_rawdata_dir")    # rå SAS-filer
workdata_dir <- getOption("fastreg.project_workdata_dir")   # parquet-output

# 2. Konvertér parallelt - brug flere cores på én gang:
plan(multisession)

# 3. Se hvilke SAS-registre der ligger i rawdata:
sas_files <- fastreg::list_sas_files()
sas_files

# 4. Konvertér til Parquet (partitioneret efter år). Placeringerne kommer fra
#    options ovenfor, så du slipper for at skrive den fulde sti:
fastreg::convert(path = sas_files[1], output_dir = workdata_dir)

# 5. Læs et register ind igen - en doven forbindelse; filtrér FØR collect():
bef <- read_register("bef") %>% rename_with(tolower)
bef %>% filter(koen == 2) %>% collect()
```

::: {.callout-note}
**Hvad er "cores" / at køre "parallelt"?** Din computers processor (CPU) har flere **cores** - uafhængige arbejdere. Som standard bruger R kun én. `plan(multisession)` (fra `furrr`) lader fastreg konvertere flere registre **på én gang**, én per core, så et helt workspace bliver færdigt meget hurtigere. Det er valgfrit - undlad det, og alt kører stadig, bare ét register ad gangen. (Hver core kører sin egen R-session, så det bruger også mere hukommelse; fint til en engangskonvertering.)
:::
...
::: {.callout-tip}
**Sæt det op én gang med en skabelon.** `fastreg::use_template()` kopierer en [`targets`](https://books.ropensci.org/targets/)-pipeline der konverterer hele dit workspace på én gang - praktisk når du har mange registre. fastreg kan også **køre konverteringen igen, når et register opdateres** med nye data, så du ikke skal bygge alt om i hånden.
:::
