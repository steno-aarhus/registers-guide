...
Du har bygget din kohorte (Fase 10), samlet dine udtræk (Fase 12) og er nu på vej til at skrive den kode der faktisk producerer dine resultater: deskriptive tabeller, modeller, sensitivitetsanalyser.
...
::: {.callout-tip}
**Kort fortalt:** Noget af det vigtigste er overblik - fx vha. ét script per trin, kør hvert script top-til-bund (aldrig på tværs), giv objekter meningsfulde navne, og kommentér *hvorfor* frem for *hvad*. Resten er finpudsning.
:::
...
Konsistent navngivning gør koden langt nemmere at læse. Vælg **snake_case** (små bogstaver med underscore) og hold dig til det:
...
- **Sektionsoverskrifter med banner-bokse:** markér afsnit med en hovedoverskrift (`#====`) og underafsnit (`#----`), som i headeren og eksemplet i afsnit 1, så et langt script er let at skanne. Vil du også have sektionerne i RStudios **outline-panel** (øverst til højre, og dropdown nederst til venstre - klik for at hoppe til en sektion), så afslut overskriftslinjen med `----` (fx `# Indlæs data ----`, indsættes hurtigt med **CTRL+SHIFT+R**). I Quarto-dokumenter bruger du markdown-overskrifter med `##`.
...
Du kan læse mere om funktioner - argumenter, defaultværdier og hvornår de betaler sig - i [Funktioner: oversigt](guide_til_funktioner.qmd).
...
På Forskermaskinen deler alle brugere den samme RAM. R indlæser data direkte i RAM, så ét stort udtræk kan bremse serveren for alle. Derfor **lukker DST automatisk processer** (RStudio-sessioner, jobs m.m.), når hukommelsen er ved at være fyldt - og bliver din session lukket, mister du alt ulagret arbejde.

::: {.callout-warning}
**Der er en grænse på 250 GB pr. brugersession** (på STATA- og R/Python-serverne). Kommer du over grænsen, bliver du **automatisk logget af** og får en mail om hændelsen. Og hvis en server samlet har under **10 % ledig hukommelse**, bliver den session med det største forbrug logget af - også selvom den er under 250 GB. Spørgsmål: [servicedesk@dst.dk](mailto:servicedesk@dst.dk) / 39 17 38 00.
:::

::: {.callout-important}
**Gem ofte, og skriv til disken undervejs.** Gem din kode løbende, og skriv mellemresultater til disk med `saveRDS()` (jf. afsnit 2 og 10), så du ikke taber timers arbejde, hvis din proces bliver lukket.
:::

Konkrete vaner der holder RAM-forbruget nede:

- **Indlæs kun det du har brug for.** Vælg kolonner og rækker *før* data lander i RAM - se [Fase 4 - Læs kun det du har brug for](04_filformater.qmd) (`open_dataset()` + `filter`/`select`, `read_sas(col_select=, n_max=)`).
...
- **Luk ubrugte sessioner**, og start en frisk session, når du går i gang med en ny opgave (`Session → Restart R`) - det rydder også gamle objekter væk.
- **Hold øje med forbruget.** Åbn genvejen **Task Manager** på serverens skrivebord → fanen **Users** → find din projektident → se forbruget under **Memory**.

Rækker det fælles miljø ikke til dine analyser, kan du undersøge en **hosted server** eller en **HPC-analyseplatform** (begge har vejledninger på DST).

::: {.callout-tip}
DST's officielle råd med kodeeksempler i R, Python og STATA er samlet i [DST-vejledning: Reduktion af RAM-forbrug i det fælles miljø (PDF)](https://www.dst.dk/Site/Dst/SingleFiles/GetArchiveFile.aspx?fi=4500415120635&fo=0&ext=forskning).
:::
...
- [Fase 12 - Saml & klargør datasættet](12_data-manipulation.qmd): joins og pivots
- [Funktioner: oversigt](guide_til_funktioner.qmd): funktioner i dybden
- [Fase 5 - Udtræk trin for trin](05_udtraek-trin-for-trin.qmd): det grundlæggende mønster
