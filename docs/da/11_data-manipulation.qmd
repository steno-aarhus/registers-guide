...
Du ankommer til denne fase med separate RDS-filer: din kohort fra [Fase 10](10_byg-din-kohorte.qmd) og de variable du har udtrukket i [Fase 11](11_foej-variabler.qmd) - udfald, socioøkonomi og komorbiditet (plus evt. demografi fra [Fase 6](06_foerste-udtraek.qmd)). Du forlader med ét analyseklart datasæt klar til [Fase 13 - Analyse](13_analyse.qmd).
...
- **Opfølgningstid** (`censor_date - index_date`) beregnes typisk *efter* joinet, fordi den bruger datoer fra flere udtræk - se [12b](12b_joins.qmd).

::: {.callout-tip}
`mutate()`, `if_else()` og `case_when()` er forklaret i [Funktioner: oversigt](15a_guide_til_funktioner.qmd).
:::
...
1. **[12a - Langt ↔ bredt format](12a_langt-bredt.qmd)** - få hvert udtræk ned til **én række per person**. Forstå forskellen på langt og bredt format, og brug enten `group_by() + slice()` (én repræsentativ værdi per person) eller `pivot_wider()` (flere tidspunkter side om side).
2. **[12b - Kobl udtræk sammen](12b_joins.qmd)** - kobl udtrækkene på `pnr` (`left_join()`, `inner_join()`, `semi_join()`, `anti_join()`), stak to versioner af samme register lodret (`bind_rows()`), validér joinet, og beregn opfølgningstid og hændelsesvariabel. **Her samles det endelige datasæt.**
...
→ [12a - Langt ↔ bredt format](12a_langt-bredt.qmd)
