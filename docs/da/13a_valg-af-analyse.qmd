...
**Parametrisk eller nonparametrisk?** En *parametrisk* test eller model antager, at data følger en bestemt fordeling (typisk normalfordeling, eller fx en binomial-antagelse i en regressionsmodel) og regner på fx gennemsnit og spredning. En *nonparametrisk* test antager ingen bestemt fordeling af de underliggende værdier - den bygger typisk på **rangordning** (for tal, fx Wilcoxon) eller på **optælling i kategorier** (for kategoriske data, fx chi-i-anden) og er mere robust over for skæve fordelinger og outliers, men har som regel lidt mindre styrke (power). Tommelfingerregel: holder den parametriske tests antagelser (se [Antagelser](#antagelser) nederst), så brug den; ellers den nonparametriske.
...
<td>Nonparametrisk</td><td>Binomialtest</td><td>Binomialfordeling; uafhængige</td><td><code>binom.test(x, n, p = p0)</code> (eksakt); <code>prop.test(x, n, p = p0)</code> (approks.)</td>
</tr>
<tr>
<td rowspan="3">Andel, to grupper</td>
<td rowspan="2">Uparret</td>
<td>Sammenlign to uparrede grupper</td>
<td>Nonparametrisk</td><td>Chi-i-anden / Fishers eksakte</td><td>Uafhængige (Fisher ved små tal)</td><td><code>chisq.test(tabel)</code>; <code>fisher.test(tabel)</code></td>
</tr>
<tr>
...
<td>Nonparametrisk</td><td>McNemar</td><td>Parrede obs.; uafhængige par</td><td><code>mcnemar.test(tabel)</code></td>
</tr>
<tr>
<td>Regression</td>
<td>Uparret</td>
