...
**Parret eller uparret?** Data er *parrede*, når målingerne hører sammen to og to - typisk den **samme person før og efter** en behandling, eller et **1:1-matchet par** (én case + én matchet kontrol). Effekten beregnes da *inden for* hvert par, og man bruger en parret test (fx paret t-test eller McNemar). Data er *uparrede*, når grupperne er **forskellige, uafhængige personer**.

**Pas på: matchet med flere kontroller er ikke "parret".** 1:1-matchning er parret, men har du matchet **1:n** (fx 1:5 case-control) eller en **matchet kohorte**, er der matchsæt (strata) frem for par - og det er ikke en parret test. Brug i stedet **betinget logistisk regression** (`clogit` med `strata()`, se [Regression](13d_regression.qmd)) for matchet case-control, eller **stratificeret/klynget Cox** for en matchet kohorte (se [Time-to-event](13e_time-to-event.qmd)). De hører altså under regression / time-to-event i tabellerne nedenfor.
