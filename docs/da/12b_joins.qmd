...
Dine udtræk fra [Fase 6](06_foerste-udtraek.qmd), [Udtræk fra LPR](09b_udtraek_fra_lpr.qmd) og [Fase 11](11_udtraek-variable.qmd) ligger som separate RDS-filer. Når hvert udtræk er nede på **én række per person** ([Langt ↔ bredt format](12a_langt-bredt.qmd)), kobler du dem sammen på `pnr` til ét analyseklart datasæt. Til sidst beregner du opfølgningstid og hændelsesvariabel.
...
Når du kobler dine **færdige person-niveau udtræk på kohorten**, er nøglen som regel `pnr`. Inde i et register er det andre nøgler: kobler du fx kontakt og diagnose i LPR, sker det på `recnum` eller `dw_ek_kontakt`, ikke `pnr` (se [oversigten over join-nøgler i LPR](09a_forstaa_lpr.qmd)). ER-diagrammet nedenfor viser den almindelige kobling på `pnr` - udtræk koblet på kohorten:
...
**Konkurrerende risici?** Skal en konkurrerende hændelse (typisk død) håndteres særskilt (se [Time-to-event](13e_time-to-event.qmd#konkurrerende-risici)), så lav i stedet for `event` (0/1) en **status med tre værdier** - hvad der skete *først*:
...
Nu er din **kohorte** bygget i [Fase 10](10_byg-studiepopulation.qmd): en tabel med én række per person, `pnr` og `index-dato` (og `exposed`/`case`, hvis du har en sammenligningsgruppe). Dine **udfald, kovariater og censurerings-variable** er trukket ud som separate RDS-filer ([Fase 9](09b_udtraek_fra_lpr.qmd), [Fase 11](11_udtraek-variable.qmd)). Dette sidste trin kobler dem på kohorten med `left_join()`, beregner opfølgningstid og beholder kun de kolonner analysen kræver.

::: {.callout-note}
**Kohorteopbygningen - inkl. matchingen - hører til [Fase 10](10_byg-studiepopulation.qmd).** Her antager vi, at kohorten allerede findes; denne side samler kun analysedatasættet rundt om den. Designvalgene (sammenligningskohorte, immortal time, matching-ratio) afgøres tilbage i [Fase 1](01_studieforberedelse.qmd#sammenligningskohorten-komparator).
:::
...
- [Langt ↔ bredt format](12a_langt-bredt.qmd): få hvert udtræk ned til én række per person
...
- [Joining data](https://www.epirhandbook.com/en/new_pages/joining_matching.html) i *The Epidemiologist R Handbook*.
- [Joins](https://r4ds.hadley.nz/joins.html) i *R for Data Science*: nøgler, mutating vs. filtering joins og hvad der sker, når en nøgle har dubletter.
:::
