...
::: {.callout-note}
Koden bruger `inner_join()` og `bind_rows()`. Er det nye begreber, er de forklaret grundigt i [Fase 11 — Joins og pivots](11_data-manipulation.qmd) — du kan sagtens følge koden her og vende tilbage til Fase 11 bagefter.
:::
...
| | Tilgang 2 — direkte udtræk | Tilgang 1 — `alle_dx` |
|---|---|---|
| **Bedst når** | Du har **ét enkelt udfald** | Du har **flere udfald** fra LPR |
...
<details>
<summary>Tilgang 2 — hent bestemte diagnoser direkte (start her ved ét udfald)</summary>
...
::: {.callout-note}
**Har du F-koder (fx demens, depression)?** Tilpas regex til at inkludere dem, fx `"^DE1[0-4]|^DF0[0-3]|^DG30"`, og tilføj psykiatrisk LPR2 — se Tilgang 1 nedenfor for kode.
:::
...
<details>
<summary>Tilgang 1 — hent alle diagnoser + filtrer udfald (ved flere udfald)</summary>
...
→ **Udfør "Ekskluder prævalente tilfælde" nedenfor, og fortsæt derefter med Del 2.**

---

**Del 2 — filtrer dit specifikke udfald** *(kræver `kohort_renset` fra Ekskluder-steget)*
...
udfald <- alle_dx %>%
  ...
  inner_join(kohort_renset %>% select(pnr, index_date), by = "pnr") %>%
