...
| **Workflow** | Hent alle → Ekskluder → filtrer per udfald | Hent specifikke koder → Ekskluder → færdig |
| **Fordel** | LPR forespørges kun én gang; genbrug til alle udfald | Simplere og hurtigere ved enkeltudtræk |

**Tilgang 1** er bedst når dit studie har flere udfald — fx hændelsesdato for diabetes, hjertesygdom og død. Du forespørger LPR én enkelt gang og bygger `alle_dx`: en fælles tabel med alle A- og B-diagnoser. For hvert nyt udfald filtrerer du blot `alle_dx` på koderne — den eneste linje du ændrer er kode-listen.

**Tilgang 2** er bedst ved ét enkelt udfald. Du filtrerer på specifikke ICD-koder direkte i `filter()`-steget *inden* `collect()`. DuckDB/Arrow sender filteret ned til lagringsformatet — kun relevante rækker hentes i RAM.

::: {.callout-note}
Eksemplerne kræver parquet-filer og en færdigbygget studiepopulation. `kohort` er data.frame med `pnr` og `index_date` per person — se [Fase 10](10_byg-din-kohorte.qmd). Er registrene i SAS, konverter dem først: [Fase 4 — SAS til parquet](04_filformater.qmd#konvertér-sas-til-parquet).
:::
...
::: {.callout-note}
**Har du F-koder (fx demens, depression)?** Psykiatriske diagnoser stillet *inden* marts 2019 er i separate registre og indgår ikke i `lpr2_dx`. Tilføj dem inden `bind_rows()`:
...
# diagnoser = alle_dx (Tilgang 1) eller dit direkte udtræk (Tilgang 2)
