...
Bemærk skrivemåden: `coxph()` tager klyngevariablen som et **argument** uden tilde (`cluster = pnr`), mens `sandwich::vcovCL()` på [Regression](13d_regression.qmd#klyngebaserede-standardfejl) bruger en **formel** med tilde (`cluster = ~ pnr`). Det er samme idé (klynge på person-id), kun to funktioners forskellige måder at modtage variablen på.
...
Selve `status_kode` (0/1/2) bygger du fra datoerne - hvad der skete *først*: 0 = censureret (fx opfølgningens slut, emigreret eller på anden vis tabt), 1 = udfald, 2 = død. Konstruktionen ligger i [Beregn opfølgningstid og hændelsesvariabel](12b_joins.qmd#opfoelgningstid) (Fase 12). Hvilket *tal* censurering får, er underordnet - det afgørende er, at det censurerede niveau står **først** i `levels =` (derfor er det kodet 0 her).
