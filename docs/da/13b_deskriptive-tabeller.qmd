...
Registerartikler rapporterer ofte den **rå (crude) incidensrate** per eksponeringsgruppe ved siden af Table 1: antal hændelser delt med samlet opfølgningstid (personår), skaleret til per 1000 personår. Det kræver `event` (1 = hændelse, 0 = censureret) og `followup_years` fra dit analyseklare datasæt (bygget i [Fase 12](12b_joins.qmd#opfoelgningstid)).
...
Det er en **deskriptiv** rate uden justering, og tæller som aggregeret output (tjek for små celler inden eksport). Vil du sammenligne grupperne med kontrol for confounding, hører det til [regression](13d_regression.qmd) og [time-to-event](13e_time-to-event.qmd), hvor en Cox-model giver en hazard ratio.
