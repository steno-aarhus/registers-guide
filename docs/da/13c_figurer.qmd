...
Du bygger en `ggplot` op ved at lægge lag og indstillinger til med `+`. Nogle af de mest brugte:

- **Farver:** `fill =` (udfyldning) og `color =` (kantlinje/punkter) inde i `aes()` farvelægger efter en variabel. Vil du selv vælge farverne, tilføjer du `scale_fill_manual(values = c("..."))`; eller brug en færdig palet, fx `scale_fill_brewer()`.
- **Akser og tekster:** `labs()` til titel og akse-/legende-tekster; `scale_y_continuous()` / `scale_x_continuous()` til akse-inddeling og grænser; `lims()` til at sætte min/max.
- **Tema:** `theme_minimal()`, `theme_bw()` mv. giver forskellige helhedsudtryk; `base_size =` skalerer al tekst; `theme(legend.position = "bottom")` flytter eller fjerner signaturforklaringen.
- **Layout:** `coord_flip()` lægger søjlerne vandret (godt til mange kategorier); `facet_wrap(~ variabel)` deler figuren op i ét panel per gruppe.

Slå et lag op med `?geom_col`, `?scale_fill_manual` osv. for at se alle argumenter.
