...
Alle udtræk i de øvrige faser - udfald ([Fase 9](09b_udtraek_fra_lpr.qmd)), kovariater ([Fase 6](06_foerste-udtraek.qmd)) og socioøkonomi ([Socioøkonomiske variable](13_socioekonomiske-variable.qmd)) - forudsætter at du allerede har en kohorte: en tabel med `pnr` og `index_date` per person. Denne side viser, hvordan du identificerer din studiepopulation; derefter vælger du studiedesign.
...
- **Prævalensstudie / kohorte uden sammenligningsgruppe** - du er færdig her; gå videre til [Fase 11 - Udtræk variable](11_foej-variabler.qmd) og [Fase 12](11_data-manipulation.qmd).
...
`inner_join()` og `bind_rows()` er vist i [Fase 9](09b_udtraek_fra_lpr.qmd). `group_by() + slice()` er vist kortfattet der og forklares grundigt i [Fase 12](11_data-manipulation.qmd). Nyt i Trin 2: `anti_join()`, `distinct()`, `pull()` samt `nrow()`, `cat()` og `stopifnot()` - se [Guide til funktioner](15a_guide_til_funktioner.qmd).
:::
...
::: {.callout-note}
**En lille hjælpe-funktion.** Skal samme tjek bruges flere gange (fx også på en sammenligningsgruppe senere, se [Fase 10a](10a_sammenligningskohorte.qmd)), kan du pakke logikken i en funktion, så de to tjek ikke skrider fra hinanden. Se [God kode-praksis](12_god-kode-praksis.qmd).
:::
...
- **Prævalensstudie / kohorte uden sammenligningsgruppe** - du har nu din studiepopulation med en index-dato. Spring videre til [Fase 11 - Udtræk variable](11_foej-variabler.qmd) og [Fase 12 - Saml & klargør datasættet](11_data-manipulation.qmd).
- **Kohortestudie med sammenligningskohorte** → [Sammenligningskohorte](10a_sammenligningskohorte.qmd) - byg en matchpool og risk-set match en sammenligningsgruppe.
