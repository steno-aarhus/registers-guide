...
subtitle: "group_by() + slice(), joins og bind_rows() — saml dine udtræk til ét datasæt"
date: last-modified
---

Dine udtræk fra [Fase 6](06_foerste-udtraek.qmd), [Fase 9](09b_udtraek_fra_lpr.qmd) og [Fase 11](11_foej-variabler.qmd) ligger som separate RDS-filer — nogle med én række per person, andre med mange. Her reducerer du hvert udtræk til **én række per person** og kobler dem på `pnr` til ét analyseklart datasæt. Til sidst beregner du opfølgningstid og hændelsesvariabel.
...
`left_join()` virker forudsigeligt, når begge tabeller har **én række per `pnr`**. Mange registerudtræk har flere rækker per person (langt format — se [12b — Langt ↔ bredt format](12b_pivots.qmd)). `group_by() + slice()` er den primære måde at reducere et datasæt til én repræsentativ række per person inden join — **det er forberedelse til join, ikke selve join**.
...
Vil du have **to eller flere tidspunkter side om side** som separate kolonner i analysen — brug `pivot_wider()` ([12b](12b_pivots.qmd)).
...
- [12b — Langt ↔ bredt format](12b_pivots.qmd) — skift en tabels form med `pivot_wider()` / `pivot_longer()`
