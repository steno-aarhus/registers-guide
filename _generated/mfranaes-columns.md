<!-- Generated from schema/registers/mfranaes.yaml by tools/build-schema-tables.R. Do not edit by hand. -->
...
- **`SKSKODE`:** No `code_system` attached: an open SKS code space restricted (by the register's own name and its correspondence to mfr.yaml's markoer_anaestesi_til_operation) to codes relevant to anaesthesia for an operation, but the exact restriction is not documented by DST - same reasoning as mfr_nyfoedte.yaml's Kejsersnit column.
